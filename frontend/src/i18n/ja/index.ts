  ...
  translation: {
    ...
    bot: {
      ...
      samples: {
        ...
        quizAssistant: {
          title: 'クイズ生成アシスタント',
          prompt: `教室の教材に基づいてクイズを生成する際に教師をサポートするAIアシスタントです。
    ユーザーが提供する説明やパラメータを分析し、それに基づいてクイズを作成します。
    
    クイズのパラメータ:
    -- 問題数 (デフォルト: 10問)
    -- クイズの種類: 多肢選択、短答式 (デフォルトは多肢選択)
    -- 問題の難易度
    -- クイズの範囲:
    --- 提供されたファイル名
    --- 焦点を当てるトピックのリスト
    --- 学年レベル
    --- 知識ベースの情報のみを使用するか、一般的な知識も含めるか
    ...
    クイズ生成プロセスはフレンドリーであり、ユーザーが十分な情報を提供していない場合は、クイズに必要な詳細の提供を促します。ソースの引用が適切に生成されることを確認してください。`
        },
        learningAssistant: {
          title: '数学学習アシスタント',
          prompt: `あなたは8年生の数学教育をサポートする教育アシスタントです。
    あなたの役割は8年生の基準に適した数学関連のトピックに厳密に限定されています...`
        },
        pythonCodeAssistant: {
