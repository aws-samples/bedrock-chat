...
{citation_prompt}"""
        ...
        agent = Agent(
            model=model, tools=[enhanced_calculator], system_prompt=system_prompt
        )
...
{citation_prompt}"""
        ...
        # Create agent with citation-enhanced tool
        agent = Agent(
            model=model, tools=[enhanced_simple_list], system_prompt=system_prompt
        )
...
def test_citation_prompt_effectiveness():
    ...
    key_phrases = ["source_id", "[^xxx]", "[source_id:", "tool result"]
...
if __name__ == "__main__":
    ...
    try:
        ...
        if success1 and success2:
            ...
            print(
                "- Check if citation prompt needs adjustment for different tool types"
            )
