        ...
        try:
            ...
            if trace_logs:
                ...
                for formatted_trace in formatted_traces:
                    ...
                    recipient = formatted_trace.get("input").get("recipient", None) if formatted_trace.get("input") != None else None
                    name = formatted_trace.get("name")
                    # swith type
                    if type == "tool_use":
                        if recipient != None:
                            result.append({
                                "content": json.dumps(formatted_trace.get("input").get("content"), default=str),
                        ...
                        if "<thinking>" in formatted_trace.get("text"):
                            result.append({
                                "content": json.dumps(formatted_trace.get("text"), default=str),
...
def _bedrock_agent_invoke(
    ...
    logger.info(f"Bedrock Agent request - Query: {input_text}, Session ID: {session_id}")
    ...
    bedrock_tool = next((tool for tool in bot.agent.tools if tool.tool_type == "bedrockAgent"), None)
