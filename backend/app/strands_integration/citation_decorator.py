...
def with_citation_support(
    display_citation: bool = False, tool_use_id: str = None
) -> Callable[[F], F]:
    """
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(
            ...
            logger.debug(
                f"[CITATION_DECORATOR] Executing tool function with citation support"
            )
            logger.debug(
                f"[CITATION_DECORATOR] display_citation: {display_citation}, tool_use_id: {tool_use_id}"
            )
            ...
            if display_citation and tool_use_id:
                ...
                logger.debug(
                    f"[CITATION_DECORATOR] Enhanced result with citation: {type(enhanced_result)}"
                )
                return enhanced_result
            else:
                logger.debug(
                    f"[CITATION_DECORATOR] Citation not enabled, returning original result"
                )
                return result
    ...
    if isinstance(result, str):
        ...
        try:
            ...
            if isinstance(parsed, dict):
                ...
                if found_list:
                    logger.debug(
                        f"[CITATION_DECORATOR] Found list in '{found_key}' with {len(found_list)} items"
                    )
                    ...
                    for i, item in enumerate(found_list):
                        ...
                        logger.debug(
                            f"[CITATION_DECORATOR] Enhanced item {i} with metadata: {item_source_id}"
                        )

                    # Join all items with newlines
                    enhanced_content = "\n".join(enhanced_items)
                    logger.debug(
                        f"[CITATION_DECORATOR] Enhanced JSON with list: {len(enhanced_items)} items"
                    )
                    ...
                    logger.debug(
                        f"[CITATION_DECORATOR] Enhanced JSON dict with single source_id: {tool_use_id}"
                    )
                    return enhanced_content
                ...
                for i, item in enumerate(parsed):
                    ...
                    enhanced_items.append(enhanced_item)
                    logger.debug(
                        f"[CITATION_DECORATOR] Enhanced list item {i} with source_id: {item_source_id}"
                    )

                enhanced_content = "\n".join(enhanced_items)
                logger.debug(
                    f"[CITATION_DECORATOR] Enhanced direct list: {len(enhanced_items)} items"
                )
                return enhanced_content
            else:
                # Other JSON types
                enhanced_content = f"{result} [source_id: {tool_use_id}]"
                logger.debug(
                    f"[CITATION_DECORATOR] Enhanced JSON with single source_id: {tool_use_id}"
                )
                return enhanced_content
            ...
            enhanced_content = f"{result} [source_id: {tool_use_id}]"
            logger.debug(
                f"[CITATION_DECORATOR] Enhanced plain string with source_id: {tool_use_id}"
            )
            return enhanced_content
        ...
        logger.debug(
            f"[CITATION_DECORATOR] Enhanced dict result with embedded source_id: {tool_use_id}"
        )
        return enhanced_content
        ...
        for i, item in enumerate(result):
            ...
            enhanced_items.append(enhanced_item)
            logger.debug(
                f"[CITATION_DECORATOR] Enhanced list item {i} with embedded source_id: {item_source_id}"
            )

        # Join all items with newlines
        enhanced_content = "\n".join(enhanced_items)
        logger.debug(
            f"[CITATION_DECORATOR] Enhanced list result with {len(enhanced_items)} items"
        )
        return enhanced_content
        ...
        logger.debug(
            f"[CITATION_DECORATOR] Returning result as-is for type: {type(result)}"
        )
        return result
