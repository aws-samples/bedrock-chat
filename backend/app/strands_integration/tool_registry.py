...
def convert_strands_args_kwargs_to_tool_params(tool_func, strands_input: dict) -> dict:
    ...
    if isinstance(main_arg_value, str):
        try:
            ...
            if isinstance(parsed_args, list) and len(parsed_args) > 0:
                ...
                logger.debug(
                    f"[TOOL_REGISTRY] Extracted main arg from JSON array: {main_arg_value}"
                )
                ...
                for i, arg_value in enumerate(parsed_args[1:], start=1):
                    if i < len(param_names):
                        ...
                        if param_name not in strands_kwargs:
                            ...
                            logger.debug(
                                f"[TOOL_REGISTRY] Mapped arg {i} to {param_name}: {arg_value}"
                            )
    ...
    merged_kwargs = {
        k: v for k, v in strands_input.items() if k not in ["args", "kwargs"]
    }
    ...
    if param_names:
        ...
        logger.debug(
            f"[TOOL_REGISTRY] Mapped args to '{main_param_name}': {main_arg_value}"
        )
    else:
    ...
    if len(filtered_kwargs) != len(merged_kwargs):
        ...
        logger.debug(
            f"[TOOL_REGISTRY] Ignored unsupported parameters: {ignored_params}"
        )
...
def get_tools_for_bot(bot: Optional[BotModel], display_citation: bool = False) -> list:
    ...
    if not bot or not bot.is_agent_enabled():
        logger.debug(
            f"[TOOL_REGISTRY] No bot or agent not enabled, returning empty tools"
        )
        return tools
    ...
    available_tools = {
        ...
        "bedrock_agent": lambda bot: create_bedrock_agent_tool(bot),
    ...
    for tool_config in bot.agent.tools:
        ...
        if tool_name in available_tools:
            ...
            logger.debug(
                f"[TOOL_REGISTRY] Added Strands tool: {tool_name} (citation: {display_citation})"
            )
    ...
    if bot.has_knowledge():
        ...
        logger.debug(
            f"[TOOL_REGISTRY] Added Strands knowledge tool (citation: {display_citation})"
        )
...
def _add_citation_support(strands_tool, tool_name: str):
    ...
    original_func = (
        strands_tool._func if hasattr(strands_tool, "_func") else strands_tool
    )
    ...
    def citation_wrapper(*args, **kwargs):
        ...
        try:
            ...
            if "args" in kwargs and "kwargs" in kwargs:
                ...
                converted_kwargs = convert_strands_args_kwargs_to_tool_params(
            ...
            logger.debug(
                f"[TOOL_REGISTRY] Enhanced result with citation: {type(enhanced_result)}"
            )
    ...
    citation_wrapper.__doc__ = getattr(
        original_func, "__doc__", f"Enhanced {tool_name} with citation support"
    )
