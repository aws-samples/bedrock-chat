...
class TestBedrockAgentTool(unittest.TestCase):
    def setUp(self):
        ...
        try:
            ...
            alias_response = self.bedrock_agent_client.create_agent_alias(
                ...
                agentAliasName=f"test-alias-{self.test_id}"
            )
    ...
    def _wait_for_agent_status(self, agent_id, expected_status, timeout=300):
        ...
        while time.time() - start_time < timeout:
            ...
            if status == expected_status:
                ...
                raise Exception(f"Agent creation failed: {response['agent'].get('failureReasons', [])}")
            
            time.sleep(5)
        
        raise Exception(f"Timeout waiting for agent {agent_id} to reach {expected_status}")
    ...
    def _wait_for_alias_status(self, agent_id, alias_id, expected_status, timeout=300):
        ...
        while time.time() - start_time < timeout:
            ...
            if status == expected_status:
                ...
                raise Exception(f"Alias creation failed: {response['agentAlias'].get('failureReasons', [])}")
            
            time.sleep(5)
        
        raise Exception(f"Timeout waiting for alias {alias_id} to reach {expected_status}")
    ...
    def test_create_tool_with_no_bot(self):
        ...
        self.assertIsNotNone(tool)
        self.assertIn("Invoke Bedrock Agent for specialized tasks", tool._tool_spec["description"])
    ...
    def test_tool_invocation_with_no_bot(self):
        ...
        self.assertIn("Bedrock Agent requires bot configuration", result["content"][0]["text"])
