...
1. Consultez [Déployer avec CDK](../README.md#deploy-using-cdk) pour le déploiement dans l'environnement AWS.
...
```sh
cd backend
python3 -m venv .venv  # Optionnel (Si vous ne voulez pas installer poetry sur votre environnement)
source .venv/bin/activate  # Optionnel (Si vous ne voulez pas installer poetry sur votre environnement)
pip install poetry
poetry install
```
