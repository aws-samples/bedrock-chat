...
我們已引入 GitHub 工作流程進行型別檢查和程式碼風格檢查。這些會在建立 Pull Request 時執行，但在程式碼風格檢查完成之前等待是不利於開發體驗的。因此，這些程式碼風格檢查任務應在提交階段自動執行。我們已引入 [Lefthook](https://github.com/evilmartians/lefthook?tab=readme-ov-file#install) 作為實現此目的的機制。這不是強制性的，但我們建議採用以獲得高效的開發體驗。此外，雖然我們不強制使用 [Prettier](https://prettier.io/) 進行 TypeScript 格式化，但我們希望您在貢獻時能採用它，因為這有助於避免代碼審查時的不必要差異。

### 安裝 Lefthook

請參考[此處](https://github.com/evilmartians/lefthook#install)。如果您是 Mac 和 Homebrew 用戶，只需運行 `brew install lefthook`。
...
更多詳情，請查看 [backend README](../backend/README_zh-TW.md)。
