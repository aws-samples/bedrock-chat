...
> [!Warning]
>
> **V2 已发布。更新时，请仔细查看[迁移指南](./migration/V1_TO_V2_zh-CN.md)。如不小心处理，**V1 版本的机器人将无法使用。**

一个使用 [Amazon Bedrock](https://aws.amazon.com/bedrock/) 提供的大语言模型（LLM）进行生成式 AI 的多语言聊天机器人。
...
添加您自己的指令，并提供外部知识作为 URL 或文件（又称为[检索增强生成（RAG）](https://aws.amazon.com/what-is/retrieval-augmented-generation/)）。机器人可以在应用程序用户之间共享。自定义机器人还可以发布为独立的 API（请参见[详情](./PUBLISH_API_zh-CN.md)）。
...
- 通过以下命令进行部署。如果您想指定要部署的版本或需要应用安全策略，请从[可选参数](#optional-parameters)中指定适当的参数。
...
- **--enable-lambda-snapstart**：启用 [Lambda SnapStart](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html)（默认：禁用）。如果设置此标志，可以改善 Lambda 函数的冷启动时间，提供更快的响应时间，从而带来更好的用户体验。
...
> [!重要]
> 如果不设置可选参数，此部署方法允许任何知道 URL 的人注册。对于生产使用，强烈建议添加 IP 地址限制并禁用自注册，以减轻安全风险（您可以定义 allowed-signup-email-domains 以限制用户，使只有来自公司域的电子邮件地址可以注册）。在执行 ./bin 时同时使用 ipv4-ranges 和 ipv6-ranges 进行 IP 地址限制，并通过使用 disable-self-register 禁用自注册。
...
- [Amazon Cognito](https://aws.amazon.com/cognito/)：用户身份验证
...
> [!重要]
> 如果在部署过程中本地环境存储空间不足，CDK 引导可能会导致错误。如果在 Cloud9 等环境中运行，建议在部署前扩展实例的卷大小。
...
2. **资源命名**：
   - 只有部分资源在名称中接收环境前缀（例如，`dev_ddb_export` 表，`dev-FrontendWebAcl`）
...
5. **环境要求**：要创建"default"以外的环境，必须使用 `parameter.ts`。仅 `-c envName` 选项是不够的，需要相应的环境定义。

6. **资源隔离**：每个环境都创建自己的资源集，允许您在同一个 AWS 账户中拥有开发、测试和生产环境，且不会发生冲突。
...
> [!重要]
> 该项目专注于 Anthropic Claude 模型，Mistral 模型支持有限。例如，提示示例是基于 Claude 模型的。这是一个仅限 Mistral 的选项，一旦启用 Mistral 模型，您只能对所有聊天功能使用 Mistral 模型，而不能同时使用 Claude 和 Mistral 模型。

### 配置默认文本生成

用户可以在自定义机器人创建界面调整[文本生成参数](https://docs.anthropic.com/claude/reference/complete_post)。如果未使用机器人，将使用 [config.py](./backend/app/config.py) 中设置的默认参数。
...
该资源使用 [i18next-browser-languageDetector](https://github.com/i18next/i18next-browser-languageDetector) 自动检测语言。您可以从应用程序菜单切换语言。另外，您可以使用查询字符串设置语言，如下所示。

> `https://example.com?lng=ja`

### 禁用自助注册

该示例默认启用自助注册。要禁用自助注册，请打开 [cdk.json](./cdk/cdk.json) 并将 `selfSignUpEnabled` 设置为 `false`。如果配置了[外部身份提供商](#外部身份提供商)，该值将被忽略并自动禁用。

### 限制注册邮箱地址的域名

默认情况下，该示例不限制注册邮箱地址的域名。要仅允许来自特定域名的注册，请打开 `cdk.json` 并在 `allowedSignUpEmailDomains` 中指定域名列表。
...
[cdk.json](./cdk/cdk.json) 中的 `enableRagReplicas` 选项控制 RAG 数据库（特别是使用 Amazon OpenSearch Serverless 的知识库）的副本设置。
...
感谢您考虑为此仓库做出贡献！我们欢迎 bug 修复、语言翻译（i18n）、功能增强、[代理工具](./docs/AGENT.md#如何开发自己的工具)和其他改进。

对于功能增强和其他改进，**在创建拉取请求之前，我们非常感谢您能创建一个功能请求 Issue 来讨论实现方法和细节。对于 bug 修复和语言翻译（i18n），可以直接创建拉取请求。**
...
本库遵循 MIT-0 许可证。请参见 [LICENSE 文件](./LICENSE)。