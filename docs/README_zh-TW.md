...
添加您自己的指令，並提供外部知識作為 URL 或文件（又稱為[RAG](https://aws.amazon.com/what-is/retrieval-augmented-generation/)）。該機器人可以在應用程序用戶之間共享。自定義機器人還可以發布為獨立的 API（請參閱[詳情](./PUBLISH_API_zh-TW.md)）。
...
通過使用[代理功能](./AGENT_zh-TW.md)，您的聊天機器人可以自動處理更複雜的任務。例如，要回答用戶的問題，代理可以從外部工具中檢索必要的信息，或將任務分解為多個步驟進行處理。
...
- 在 us-east-1 區域，開啟 [Bedrock 模型存取](https://us-east-1.console.aws.amazon.com/bedrock/home?region=us-east-1#/modelaccess) > `管理模型存取` > 勾選 `Anthropic / Claude 3` 的所有模型、`Amazon / Nova`、`Amazon / Titan Text Embeddings V2` 和 `Cohere / Embed Multilingual`，然後點選 `儲存變更`。
...
- 執行以下命令進行部署。如果您想指定要部署的版本或需要套用安全性原則，請從[可選參數](#可選參數)中指定適當的參數。
...
- 系統會詢問您是新使用者還是使用 v2 版本。如果您不是 v0 的延續使用者，請輸入 `y`。
...
- **--enable-lambda-snapstart**：啟用 [Lambda SnapStart](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html)（預設：停用）。如果設定此標誌，可改善 Lambda 函數的冷啟動時間，提供更快的回應速度，帶來更好的使用者體驗。
...
> [!注意]
> 覆寫值將在 AWS 代碼建置時與現有的 cdk.json 配置合併。指定的覆寫值將優先於 cdk.json 中的值。
...
如上圖所示，將出現登入畫面，您可以在此註冊電子郵件並登入。

> [!重要]
> 如果不設定可選參數，此部署方法允許任何知道 URL 的人註冊。對於生產環境使用，強烈建議添加 IP 位址限制並禁用自行註冊，以減輕安全風險（您可以定義 allowed-signup-email-domains 以限制使用者，使只有您公司網域的電子郵件地址可以註冊）。在執行 ./bin 時，同時使用 ipv4-ranges 和 ipv6-ranges 進行 IP 位址限制，並使用 disable-self-register 禁用自行註冊。
...
這是一個建立在 AWS 受管服務上的架構，無需基礎設施管理。透過使用 Amazon Bedrock，無需與 AWS 外部的 API 通訊。這使得部署可擴展、可靠且安全的應用程式成為可能。
...
- [AWS Step Functions](https://aws.amazon.com/step-functions/)：編排擷取管道以將外部知識嵌入 Bedrock 知識庫
...
- 您將得到類似以下的輸出。Web 應用的 URL 將在 `BedrockChatStack.FrontendURL` 中輸出，請在瀏覽器中訪問。
...
您可以通過兩種方式為部署定義參數：使用 `cdk.json` 或使用類型安全的 `parameter.ts` 文件。
...
3. **環境標識**：
   - 所有資源都使用 `CDKEnvironment` 標籤標記，其中包含環境名稱
   - 您可以使用此標籤識別資源屬於哪個環境
...
5. **環境要求**：要創建"默認"以外的環境，必須使用 `parameter.ts`。僅使用 `-c envName` 選項是不夠的，必須有相應的環境定義。

6. **資源隔離**：每個環境都會創建自己的資源集，允許您在同一個 AWS 賬戶中擁有開發、測試和生產環境，且不會衝突。

## 其他

### 設定預設文字生成

使用者可以在自訂機器人建立畫面中調整[文字生成參數](https://docs.anthropic.com/claude/reference/complete_post)。如果機器人未被使用，將使用 [config.py](./backend/app/config.py) 中設定的預設參數。
...
如果使用 cli 和 CDK，請執行 `npx cdk destroy`。若不是，請存取 [CloudFormation](https://console.aws.amazon.com/cloudformation/home)，然後手動刪除 `BedrockChatStack` 和 `FrontendWafStack`。請注意 `FrontendWafStack` 位於 `us-east-1` 區域。
...
此範例預設啟用自助註冊。要停用自助註冊，請開啟 [cdk.json](./cdk/cdk.json) 並將 `selfSignUpEnabled` 設為 `false`。如果您設定[外部身份提供者](#外部身份提供者)，該值將被忽略並自動停用。
...
- **false**：減少副本數量以降低成本，適合開發和測試。

這是一個帳戶/區域層級的設定，影響整個應用程式而非個別機器人。

> [!注意]
> 截至 2024 年 6 月，Amazon OpenSearch Serverless 支援 0.5 OCU，降低小規模工作負載的入門成本。生產部署可以從 2 個 OCU 開始，而開發/測試工作負載可以使用 1 個 OCU。OpenSearch Serverless 會根據工作負載需求自動擴展。更多詳情請訪問[公告](https://aws.amazon.com/jp/about-aws/whats-new/2024/06/amazon-opensearch-serverless-entry-cost-half-collection-types/)。
...
[Lambda SnapStart](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html) 改善 Lambda 函數的冷啟動時間，提供更快的響應時間以改善使用者體驗。另一方面，對於 Python 函數，根據快取大小有[相關費用](https://aws.amazon.com/lambda/pricing/#SnapStart_Pricing)，且[目前在某些區域不可用](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html#snapstart-supported-regions)。要停用 SnapStart，請編輯 `cdk.json`。
...
- `hostedZoneId`：將創建網域記錄的 Route 53 託管區域 ID
...
對於功能增強和其他改進，**在創建 Pull Request 之前，我們非常感謝您先創建功能請求 Issue 來討論實施方法和細節。對於錯誤修復和語言翻譯（i18n），可以直接創建 Pull Request。**
...
此程式庫依照 MIT-0 授權條款授權。請參見 [LICENSE 檔案](./LICENSE)。