...
添加您自己的指令，並提供外部知識作為 URL 或檔案（又稱為[檢索增強生成（RAG）](https://aws.amazon.com/what-is/retrieval-augmented-generation/)）。該機器人可以在應用程式用戶之間共享。自定義機器人還可以發佈為獨立的 API（請參見[詳情](./PUBLISH_API_zh-TW.md)）。
...
- 在 us-east-1 區域，開啟 [Bedrock 模型存取](https://us-east-1.console.aws.amazon.com/bedrock/home?region=us-east-1#/modelaccess) > `管理模型存取` > 勾選所有 `Anthropic / Claude 3`、所有 `Amazon / Nova`、`Amazon / Titan Text Embeddings V2` 和 `Cohere / Embed Multilingual`，然後 `儲存變更`。
...
- 使用以下命令進行部署。如果您想要指定部署的版本或需要套用安全性原則，請從[可選參數](#optional-parameters)中指定適當的參數。
...
- **--enable-lambda-snapstart**：啟用 [Lambda SnapStart](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html)（預設：停用）。如果設定此標誌，可改善 Lambda 函數的冷啟動時間，提供更快的回應時間，提升使用者體驗。
...
- **--repo-url**：要部署的 Bedrock Claude Chat 自訂儲存庫，如果是分支或自訂原始碼控制。（預設：https://github.com/aws-samples/bedrock-claude-chat.git）
...
> [!重要]
> 如果不設置可選參數，此部署方法允許任何知道 URL 的人註冊。對於生產使用，強烈建議添加 IP 位址限制並停用自行註冊，以降低安全風險（您可以定義 allowed-signup-email-domains 以限制使用者，使只有公司網域的電子郵件地址可以註冊）。在執行 ./bin 時，同時使用 ipv4-ranges 和 ipv6-ranges 進行 IP 位址限制，並使用 disable-self-register 停用自行註冊。
...
超級簡單的部署使用 [AWS CodeBuild](https://aws.amazon.com/codebuild/) 透過 CDK 內部執行部署。本節描述直接使用 CDK 進行部署的程序。

- 請確保已安裝 UNIX、Docker 和 Node.js 運行環境。如果沒有，您也可以使用 [Cloud9](https://github.com/aws-samples/cloud9-setup-for-prototyping)

> [!重要]
> 如果在本地環境部署期間儲存空間不足，CDK 引導可能會導致錯誤。如果您在 Cloud9 等環境中運行，建議在部署前擴展實例的磁碟大小。
...
使用者可以在自定義機器人創建畫面調整[文字生成參數](https://docs.anthropic.com/claude/reference/complete_post)。如果機器人未被使用，則會使用 [config.py](./backend/app/config.py) 中設置的預設參數。
...
如果使用 CLI 和 CDK，請使用 `npx cdk destroy`。如果不是，請訪問 [CloudFormation](https://console.aws.amazon.com/cloudformation/home)，然後手動刪除 `BedrockChatStack` 和 `FrontendWafStack`。請注意 `FrontendWafStack` 位於 `us-east-1` 區域。

### 語言設定

此資源使用 [i18next-browser-languageDetector](https://github.com/i18next/i18next-browser-languageDetector) 自動偵測語言。您可以從應用程式選單切換語言。另外，您也可以使用查詢字串設置語言，如下所示。

> `https://example.com?lng=ja`

### 禁用自助註冊

此範例預設啟用自助註冊。要禁用自助註冊，請打開 [cdk.json](./cdk/cdk.json) 並將 `selfSignUpEnabled` 設為 `false`。如果您配置了[外部身份提供者](#external-identity-provider)，則該值將被忽略並自動禁用。

### 限制註冊郵件地址的網域

預設情況下，此範例不限制註冊郵件地址的網域。要僅允許特定網域的註冊，請打開 `cdk.json` 並在 `allowedSignUpEmailDomains` 中指定網域列表。
...
此範例支持外部身份提供者。目前支持 [Google](./idp/SET_UP_GOOGLE_zh-TW.md) 和[自定義 OIDC 提供者](./idp/SET_UP_CUSTOM_OIDC_zh-TW.md)。
...
如果您希望新創建的用戶自動加入群組，可以在 [cdk.json](./cdk/cdk.json) 中指定。

```json
"autoJoinUserGroups": ["CreatingBotAllowed"],
```

預設情況下，新創建的用戶將加入 `CreatingBotAllowed` 群組。

### 配置 RAG 副本

`enableRagReplicas` 是 [cdk.json](./cdk/cdk.json) 中的一個選項，用於控制 RAG 資料庫的副本設置，特別是使用 Amazon OpenSearch Serverless 的知識庫。

- **預設**：true
- **true**：通過啟用額外副本來增強可用性，適合生產環境，但會增加成本。
- **false**：通過減少副本來降低成本，適合開發和測試。

這是一個帳戶/區域級別的設置，影響整個應用程式，而不是個別機器人。
...
您可以通過在 [cdk.json](./cdk/cdk.json) 中設置以下參數來為 CloudFront 分發配置自定義網域：
...
- `alternateDomainName`：您的聊天應用程式的自定義網域名稱（例如 chat.example.com）
...
感謝您考慮為此儲存庫做出貢獻！我們歡迎修復錯誤、語言翻譯（i18n）、功能增強、[代理工具](./docs/AGENT.md#how-to-develop-your-own-tools)和其他改進。

對於功能增強和其他改進，**在創建 Pull Request 之前，我們非常感謝您能創建一個功能請求問題來討論實施方法和細節。對於錯誤修復和語言翻譯（i18n），可以直接創建 Pull Request。**
