...
1. 进入 Google 开发者控制台。
2. 创建新项目或选择现有项目。
3. 导航至"凭据"，然后点击"创建凭据"并选择"OAuth 客户端 ID"。
4. 如果提示，配置同意屏幕。
5. 对于应用程序类型，选择"Web 应用程序"。
6. 暂时将重定向 URI 留空，稍后设置。[请参见步骤5](#step-5-update-google-oauth-client-with-cognito-redirect-uris)
7. 创建后，记下客户端 ID 和客户端密钥。

详细信息请访问 [Google 官方文档](https://support.google.com/cloud/answer/6158849?hl=en)
...
5. 按照提示命名和描述密钥。记下密钥名称，因为您将在CDK代码中使用它。例如，googleOAuthCredentials。（在步骤3中使用变量名<YOUR_SECRET_NAME>）
6. 审核并存储密钥。
