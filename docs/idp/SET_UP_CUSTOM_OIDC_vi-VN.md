...
Thực hiện theo quy trình của nhà cung cấp OIDC mục tiêu, và ghi chú các giá trị cho ID client OIDC và bí mật. URL của nhà phát hành cũng được yêu cầu ở các bước tiếp theo. Nếu cần URI chuyển hướng trong quá trình thiết lập, hãy nhập giá trị giả, sẽ được thay thế sau khi triển khai hoàn tất.
...
5. Làm theo các lời nhắc để đặt tên và mô tả cho secret. Ghi chú tên secret vì bạn sẽ cần nó trong mã CDK của mình (Được sử dụng trong bước 3 với tên biến <YOUR_SECRET_NAME>).
