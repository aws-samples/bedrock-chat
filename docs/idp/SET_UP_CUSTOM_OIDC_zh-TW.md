...
按照目標 OIDC 提供者的程序，記下 OIDC 用戶端 ID 和密鑰的值。後續步驟還需要頒發者 URL。如果設定過程需要重新導向 URI，請先輸入虛擬值，稍後將在部署完成後替換。
...
5. 按照提示為密鑰命名和描述。記下密鑰名稱，因為您將在步驟 3 的 CDK 程式碼中使用它（在步驟 3 變數名稱 <YOUR_SECRET_NAME>）。
6. 檢查並儲存密鑰。
...
```json
{
  "context": {
    ...
    "userPoolDomainPrefix": "<您使用者池域的唯一前綴>"
  }
}
```
