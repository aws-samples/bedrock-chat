...
遵循目標 OIDC 提供者的程序，並記下 OIDC 客戶端 ID 和密鑰的值。後續步驟還需要頒發者 URL。如果設置過程需要重定向 URI，請輸入虛擬值，這將在部署完成後被替換。
...
```json
{
  "context": {
    ...
    "userPoolDomainPrefix": "<您使用者池的唯一網域前置詞>"
  }
}
```
...
`userPoolDomainPrefix` 必須在所有 Amazon Cognito 使用者中全域唯一。如果您選擇的前置詞已被其他 AWS 帳戶使用，使用者池網域的建立將會失敗。建議在前置詞中包含識別碼、專案名稱或環境名稱，以確保唯一性。
