...
遵循目標 OIDC 提供者的程序，並記下 OIDC 用戶端 ID 和密鑰的值。後續步驟還需要頒發者 URL。如果設置過程需要重新導向 URI，請輸入虛擬值，這將在部署完成後被替換。
...
3. 選擇「其他類型的密鑰」。
4. 以鍵值對的方式輸入用戶端 ID 和用戶端密鑰。

   - 鍵：`clientId`，值：<YOUR_GOOGLE_CLIENT_ID>
   - 鍵：`clientSecret`，值：<YOUR_GOOGLE_CLIENT_SECRET>
   - 鍵：`issuerUrl`，值：<ISSUER_URL_OF_THE_PROVIDER>

5. 按照提示為密鑰命名和描述。記下密鑰名稱，因為您將在 CDK 程式碼中需要它（在步驟 3 中使用變數名稱 <YOUR_SECRET_NAME>）。
6. 檢查並儲存密鑰。

### 注意

鍵名必須完全匹配字串 `clientId`、`clientSecret` 和 `issuerUrl`。
...
在部署堆疊後，`AuthApprovedRedirectURI` 將顯示在 CloudFormation 輸出中。返回您的 OIDC 配置，並使用正確的重新導向 URI 進行更新。