...
依照目標 OIDC 提供者的流程，記下 OIDC 用戶端 ID 和密鑰的值。後續步驟還需要簽發者 URL。如果設定過程需要重新導向 URI，請先輸入虛擬值，這將在部署完成後被替換。
...
```json
{
  "context": {
    ...
    "userPoolDomainPrefix": "<您用戶池域名的唯一前綴>"
  }
}
```
...
`userPoolDomainPrefix` 必須在所有 Amazon Cognito 用戶中全域唯一。如果您選擇的前綴已被另一個 AWS 帳戶使用，建立用戶池域名將會失敗。建議在前綴中包含識別碼、專案名稱或環境名稱，以確保唯一性。
