...
5. Thực hiện theo các lời nhắc để đặt tên và mô tả bí mật. Ghi chú tên bí mật vì bạn sẽ cần nó trong mã CDK của mình. Ví dụ: googleOAuthCredentials. (Sử dụng trong bước 3 tên biến <YOUR_SECRET_NAME>)
...
## Bước 5: Cập Nhật Client OAuth của Google với URI Chuyển Hướng của Cognito

Sau khi triển khai stack, AuthApprovedRedirectURI sẽ hiển thị trong các kết quả của CloudFormation. Quay lại Google Developer Console và cập nhật client OAuth với các URI chuyển hướng chính xác.