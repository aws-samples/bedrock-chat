...
Tên khóa phải khớp chính xác với các chuỗi 'clientId' và 'clientSecret'.
...
Sau khi triển khai stack, AuthApprovedRedirectURI sẽ được hiển thị trong các kết quả của CloudFormation. Hãy quay lại Google Developer Console và cập nhật OAuth client với các địa chỉ chuyển hướng chính xác.