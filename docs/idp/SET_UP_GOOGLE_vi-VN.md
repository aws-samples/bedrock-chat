...
6. Để trống URI chuyển hướng ở thời điểm này để thiết lập sau, và lưu tạm thời.[Xem Bước 5](#step-5-update-google-oauth-client-with-cognito-redirect-uris)
...
5. Làm theo các lời nhắc để đặt tên và mô tả bí mật. Lưu ý tên bí mật vì bạn sẽ cần nó trong mã CDK của mình. Ví dụ: googleOAuthCredentials. (Sử dụng trong tên biến <YOUR_SECRET_NAME> ở Bước 3)
...
Tiền tố userPoolDomainPrefix phải là duy nhất trên toàn cầu cho tất cả người dùng Amazon Cognito. Nếu bạn chọn một tiền tố đã được sử dụng bởi một tài khoản AWS khác, việc tạo domain user pool sẽ thất bại. Một thực hành tốt là bao gồm các định danh, tên dự án hoặc tên môi trường trong tiền tố để đảm bảo tính duy nhất.
...
Sau khi triển khai ngăn xếp, AuthApprovedRedirectURI sẽ được hiển thị trong các kết quả đầu ra của CloudFormation. Hãy quay lại Bảng điều khiển Nhà phát triển Google và cập nhật ứng dụng OAuth với các URI chuyển hướng chính xác.