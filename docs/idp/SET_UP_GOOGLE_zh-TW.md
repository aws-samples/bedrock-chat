...
## 步驟 2：將 Google OAuth 憑證儲存在 AWS Secrets Manager 中
...
```json
{
  "context": {
    ...
    "userPoolDomainPrefix": "<使用者池域名前綴的唯一值>"
  }
}
```
...
userPoolDomainPrefix 必須在所有 Amazon Cognito 使用者中全域唯一。如果您選擇的前綴已被其他 AWS 帳戶使用，使用者池域名的建立將會失敗。建議在前綴中包含識別碼、專案名稱或環境名稱，以確保唯一性。
