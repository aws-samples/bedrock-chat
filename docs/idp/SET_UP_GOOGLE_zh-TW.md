...
6. 暫時將重新導向 URI 保留為空白，稍後再設定。[請參見步驟5](#step-5-update-google-oauth-client-with-cognito-redirect-uris)
7. 建立後，記下用戶端 ID 和用戶端密鑰。

如需詳細資訊，請造訪 [Google 官方文件](https://support.google.com/cloud/answer/6158849?hl=en)
...
5. 依照提示為密鑰命名和描述。記下密鑰名稱，因為您將在 CDK 程式碼中使用它。例如，googleOAuthCredentials（在步驟 3 中使用變數名稱 <YOUR_SECRET_NAME>）
6. 檢查並儲存密鑰。
...
userPoolDomainPrefix 必須在所有 Amazon Cognito 使用者中全域唯一。如果您選擇的前綴已被其他 AWS 帳戶使用，則使用者池域的建立將會失敗。最好在前綴中包含識別碼、專案名稱或環境名稱，以確保唯一性。
