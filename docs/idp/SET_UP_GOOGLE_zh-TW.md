...
6. 暫時將重新導向 URI 留空，以便稍後設定，並暫時儲存。[請參見步驟 5](#step-5-update-google-oauth-client-with-cognito-redirect-uris)
...
3. 選擇「其他類型的密鑰」。
4. 以鍵值對的形式輸入 Google OAuth 的 clientId 和 clientSecret。

   1. 鍵：clientId，值：<YOUR_GOOGLE_CLIENT_ID>
   2. 鍵：clientSecret，值：<YOUR_GOOGLE_CLIENT_SECRET>

5. 按照提示命名並描述密鑰。記下密鑰名稱，因為您將在 CDK 程式碼中需要它。例如，googleOAuthCredentials。（在步驟 3 中使用變數名稱 <YOUR_SECRET_NAME>）
6. 檢查並儲存密鑰。
...
userPoolDomainPrefix 必須在所有 Amazon Cognito 使用者中全域唯一。如果您選擇的前綴已被其他 AWS 帳戶使用，使用者池域的建立將會失敗。最佳做法是在前綴中包含識別碼、專案名稱或環境名稱，以確保唯一性。
