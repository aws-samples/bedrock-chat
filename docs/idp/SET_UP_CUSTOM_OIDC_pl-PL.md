...
Postępuj zgodnie z procedurami dla docelowego dostawcy OIDC i zanotuj wartości identyfikatora klienta OIDC oraz jego sekretu. Adres URL wystawcy również będzie potrzebny w kolejnych krokach. Jeśli podczas procesu konfiguracji wymagany jest identyfikator URI przekierowania, wprowadź wartość zastępczą, która zostanie zastąpiona po zakończeniu wdrożenia.
...
## Krok 5: Aktualizacja klienta OIDC z przekierowaniami URI w Cognito

Po wdrożeniu stosu, `AuthApprovedRedirectURI` pojawi się w wynikach CloudFormation. Wróć do konfiguracji OIDC i zaktualizuj ją o prawidłowe adresy URL przekierowań.