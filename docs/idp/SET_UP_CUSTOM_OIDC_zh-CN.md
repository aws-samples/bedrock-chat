...
按照目标OIDC提供商的流程操作，并记录OIDC客户端ID和密钥的值。在后续步骤中还需要颁发者URL。如果设置过程中需要重定向URI，可以先输入一个虚拟值，该值将在部署完成后被替换。
...
键名必须精确匹配字符串 `clientId`、`clientSecret` 和 `issuerUrl`。
...
部署堆栈后，`AuthApprovedRedirectURI` 将显示在 CloudFormation 输出中。返回您的 OIDC 配置，并使用正确的重定向 URI 进行更新。