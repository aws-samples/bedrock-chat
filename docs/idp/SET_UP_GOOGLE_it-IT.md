...
6. Lasciare vuoto l'URI di reindirizzamento per ora per impostarlo successivamente, e salvare temporaneamente. [Vedi Passaggio 5](#step-5-update-google-oauth-client-with-cognito-redirect-uris)
...
5. Seguire le istruzioni per assegnare un nome e una descrizione al segreto. Annotare il nome del segreto poiché sarà necessario nel codice CDK. Ad esempio, googleOAuthCredentials. (Da utilizzare nel nome della variabile del Passaggio 3 <YOUR_SECRET_NAME>)
...
Dopo aver distribuito lo stack, AuthApprovedRedirectURI sarà visibile negli output di CloudFormation. Tornare nella Console degli Sviluppatori di Google e aggiornare il client OAuth con gli URI di reindirizzamento corretti.