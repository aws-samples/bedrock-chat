...
Segui le procedure per il provider OIDC di destinazione e prendi nota dei valori per l'ID client OIDC e il segreto. Inoltre, l'URL dell'emittente è richiesto nei passaggi successivi. Se durante il processo di configurazione è necessario un URI di reindirizzamento, inserisci un valore fittizio, che verrà sostituito dopo il completamento della distribuzione.
