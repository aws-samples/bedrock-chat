...
Kết quả từ LLM có thể không phải lúc nào cũng đáp ứng được kỳ vọng của người dùng. Đôi khi nó không thể thỏa mãn nhu cầu của người dùng. Để "tích hợp" LLM một cách hiệu quả vào hoạt động kinh doanh và cuộc sống hàng ngày, việc triển khai một vòng phản hồi là điều cần thiết. Bedrock Chat được trang bị tính năng phản hồi được thiết kế để cho phép người dùng phân tích nguyên nhân gây ra sự không hài lòng. Dựa trên kết quả phân tích, người dùng có thể điều chỉnh các prompt, nguồn dữ liệu RAG và các tham số cho phù hợp.

![](./imgs/feedback_loop.png)

![](./imgs/feedback-using-claude-chat.png)

Các nhà phân tích dữ liệu có thể truy cập nhật ký hội thoại bằng [Amazon Athena](https://aws.amazon.com/jp/athena/). Nếu họ muốn phân tích dữ liệu bằng [Jupyter Notebook](https://jupyter.org/), [notebook mẫu này](../examples/notebooks/feedback_analysis_example.ipynb) có thể được dùng làm tham khảo.

## Bảng điều khiển

Hiện tại cung cấp tổng quan cơ bản về việc sử dụng chatbot và người dùng, tập trung vào việc tổng hợp dữ liệu cho mỗi bot và người dùng trong các khoảng thời gian cụ thể và sắp xếp kết quả theo phí sử dụng.

![](./imgs/admin_bot_analytics.png)

## Ghi chú

- Như đã nêu trong phần [kiến trúc](../README.md#architecture), các tính năng quản trị sẽ tham chiếu đến bucket S3 được xuất từ DynamoDB. Xin lưu ý rằng vì việc xuất dữ liệu được thực hiện mỗi giờ một lần, các cuộc hội thoại mới nhất có thể không được phản ánh ngay lập tức.

- Trong thống kê sử dụng bot công khai, các bot chưa được sử dụng trong khoảng thời gian chỉ định sẽ không được liệt kê.

- Trong thống kê người dùng, những người dùng chưa sử dụng hệ thống trong khoảng thời gian chỉ định sẽ không được liệt kê.
...
## Tải xuống dữ liệu hội thoại

Bạn có thể truy vấn nhật ký hội thoại bằng Athena, sử dụng SQL. Để tải xuống nhật ký, mở Athena Query Editor từ bảng điều khiển quản lý và chạy SQL. Dưới đây là một số truy vấn mẫu hữu ích để phân tích các trường hợp sử dụng. Phản hồi có thể được tham chiếu trong thuộc tính `MessageMap`.
...
> [!Note]
> Phân tích việc sử dụng của người dùng sẽ sớm ra mắt.
