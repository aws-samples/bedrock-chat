...
Un Agente che utilizza ReAct può essere applicato in diversi scenari, fornendo soluzioni accurate ed efficienti.
...
3. Per attivare uno strumento, basta cambiare l'interruttore accanto allo strumento desiderato. Una volta abilitato uno strumento, l'Agente avrà accesso a esso e potrà utilizzarlo durante l'elaborazione delle query degli utenti.
...
Questo strumento dipende da [DuckDuckGo](https://duckduckgo.com/) che ha un limite di frequenza. È adatto per scopi di prova o demo, ma se si desidera utilizzarlo in un ambiente di produzione, si consiglia di utilizzare un'altra API di ricerca.
...
- Fai riferimento all'implementazione di esempio di uno [strumento per il calcolo del BMI](../examples/agents/tools/bmi/bmi.py). Questo esempio dimostra come creare uno strumento che calcola l'Indice di Massa Corporea (BMI) in base all'input dell'utente.
