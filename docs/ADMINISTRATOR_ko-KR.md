...
관리자 기능은 맞춤형 봇 사용과 사용자 행동에 대한 중요한 인사이트를 제공하는 필수적인 도구입니다. 이 기능이 없다면 관리자가 어떤 맞춤형 봇이 인기 있는지, 왜 인기가 있는지, 누가 사용하는지를 이해하기 어려울 것입니다. 이 정보는 지시 프롬프트 최적화, RAG 데이터 소스 맞춤화, 그리고 잠재적 인플루언서가 될 수 있는 주요 사용자를 식별하는 데 매우 중요합니다.

## 피드백 루프

LLM의 출력이 항상 사용자의 기대에 부합하지 않을 수 있습니다. 때때로 사용자의 요구를 충족시키지 못할 수 있습니다. LLM을 비즈니스 운영 및 일상 생활에 효과적으로 "통합"하기 위해서는 피드백 루프를 구현하는 것이 필수적입니다. Bedrock Claude Chat에는 사용자가 불만족의 이유를 분석할 수 있도록 설계된 피드백 기능이 탑재되어 있습니다. 분석 결과를 바탕으로 사용자는 프롬프트, RAG 데이터 소스 및 매개변수를 적절히 조정할 수 있습니다.
...
현재 챗봇과 사용자 사용량에 대한 기본적인 개요를 제공하며, 특정 시간 기간 동안 각 봇과 사용자의 데이터를 집계하고 사용 요금별로 결과를 정렬하는 데 중점을 둡니다.
