...
> [!Warning]
>
> **Se ha publicado la versión V2. Para actualizar, revise cuidadosamente la [guía de migración](./migration/V1_TO_V2_es-ES.md).** Sin ningún cuidado, **LOS BOTS DE LA VERSIÓN V1 SE VOLVERÁN INUTILIZABLES.**
...
Analice el uso para cada usuario / bot en el panel de administración. [detalles](./ADMINISTRATOR_es-ES.md)
...
- Se le preguntará si es un nuevo usuario o si está usando v2. Si no es un usuario continuo desde v0, ingrese `y`.

### Parámetros opcionales

Puede especificar los siguientes parámetros durante la implementación para mejorar la seguridad y la personalización:

- **--disable-self-register**: Desactivar el registro automático (predeterminado: habilitado). Si esta bandera está configurada, deberá crear todos los usuarios en Cognito y no permitirá a los usuarios registrarse por sí mismos.
...
> [!Importante]
> Sin establecer el parámetro opcional, este método de implementación permite que cualquiera que conozca la URL se registre. Para uso en producción, se recomienda encarecidamente agregar restricciones de direcciones IP y deshabilitar el registro automático para mitigar riesgos de seguridad (puede definir allowed-signup-email-domains para restringir usuarios de modo que solo se puedan registrar direcciones de correo electrónico del dominio de su empresa). Use tanto ipv4-ranges como ipv6-ranges para restricciones de direcciones IP y deshabilite el registro automático mediante disable-self-register al ejecutar ./bin.
...
- [Amazon API Gateway](https://aws.amazon.com/api-gateway/) + [AWS Lambda](https://aws.amazon.com/lambda/): Punto final de API de backend ([AWS Lambda Web Adapter](https://github.com/awslabs/aws-lambda-web-adapter), [FastAPI](https://fastapi.tiangolo.com/))
...
Este ejemplo admite un proveedor de identidad externo. Actualmente, compatibilidad con [Google](./idp/SET_UP_GOOGLE_es-ES.md) y [proveedor OIDC personalizado](./idp/SET_UP_CUSTOM_OIDC_es-ES.md).
