...
채팅, 지식 기반 커스텀 봇(RAG), 봇 스토어를 통한 봇 공유, 에이전트를 활용한 작업 자동화를 지원합니다.
...
자신만의 지시사항과 지식([RAG](https://aws.amazon.com/what-is/retrieval-augmented-generation/) 이라고도 함)을 추가할 수 있습니다. 봇은 봇 스토어 마켓플레이스를 통해 애플리케이션 사용자들과 공유할 수 있습니다. 커스터마이즈된 봇은 독립 실행형 API로도 게시할 수 있습니다([자세히 보기](./PUBLISH_API_ko-KR.md)).
...
> [!Important]
> 거버넌스상의 이유로, 허가된 사용자만 커스텀 봇을 생성할 수 있습니다. 커스텀 봇 생성을 허용하려면 사용자가 `CreatingBotAllowed`라는 그룹의 멤버여야 하며, 이는 관리 콘솔 > Amazon Cognito 사용자 풀 또는 aws cli를 통해 설정할 수 있습니다. 사용자 풀 ID는 CloudFormation > BedrockChatStack > Outputs > `AuthUserPoolIdxxxx`에서 확인할 수 있습니다.
...
봇과 지식베이스를 사용하려면 (OpenSearch Serverless가 기본 선택임) [OpenSearch Serverless와 Ingestion API를 사용할 수 있는 리전](https://docs.aws.amazon.com/general/latest/gr/opensearch-service.html)에 Bedrock Chat을 배포해야 합니다. 2025년 8월 기준으로 다음 리전들이 지원됩니다: us-east-1, us-east-2, us-west-1, us-west-2, ap-south-1, ap-northeast-1, ap-northeast-2, ap-southeast-1, ap-southeast-2, ca-central-1, eu-central-1, eu-west-1, eu-west-2, eu-south-2, eu-north-1, sa-east-1
...
- 새로운 사용자인지 또는 v3를 사용하는지 물어볼 것입니다. v0에서 계속 사용하는 사용자가 아니라면 `y`를 입력하세요.

### 선택적 파라미터

배포 시 보안 강화와 커스터마이징을 위해 다음 파라미터들을 지정할 수 있습니다:
...
- **--repo-url**: 포크되었거나 커스텀 소스 컨트롤의 경우, 배포할 Bedrock Chat의 커스텀 레포지토리. (기본값: https://github.com/aws-samples/bedrock-chat.git)
...
- `globalAvailableModels`: 활성화할 모델 ID 목록을 받습니다. 기본값은 빈 목록으로, 모든 모델을 활성화합니다.
- `logoPath`: 내비게이션 드로어 상단에 표시되는 프론트엔드 `public/` 디렉토리 내의 로고 에셋의 상대 경로.
- 그리고 cdk.json에 정의된 다른 컨텍스트 값들

> [!Note]
> override 값들은 AWS 코드 빌드에서 배포 시 기존 cdk.json 구성과 병합됩니다. override에 지정된 값들이 cdk.json의 값들보다 우선순위를 가집니다.
...
- 약 35분 후에 브라우저에서 접속할 수 있는 다음과 같은 출력을 받게 됩니다
...
위와 같이 회원가입 화면이 나타나며, 이메일을 등록하고 로그인할 수 있습니다.

> [!Important]
> 선택적 파라미터를 설정하지 않으면, 이 배포 방법은 URL을 아는 누구나 가입할 수 있습니다. 프로덕션 사용을 위해서는 보안 위험을 줄이기 위해 IP 주소 제한을 추가하고 자체 가입을 비활성화하는 것이 강력히 권장됩니다 (allowed-signup-email-domains를 정의하여 회사 도메인의 이메일 주소만 가입할 수 있도록 사용자를 제한할 수 있습니다). IP 주소 제한을 위해 ipv4-ranges와 ipv6-ranges를 모두 사용하고, ./bin 실행 시 disable-self-register를 사용하여 자체 가입을 비활성화하세요.

> [!TIP]
> `Frontend URL`이 나타나지 않거나 Bedrock Chat이 제대로 작동하지 않는 경우, 최신 버전의 문제일 수 있습니다. 이 경우 파라미터에 `--version "v3.0.0"`을 추가하고 배포를 다시 시도해보세요.
...
- [Amazon Bedrock Knowledge Bases](https://aws.amazon.com/bedrock/knowledge-bases/): 검색 증강 생성([RAG](https://aws.amazon.com/what-is/retrieval-augmented-generation/))을 위한 관리형 인터페이스를 제공하며, 문서 임베딩 및 구문 분석 서비스 제공
...
- [Amazon OpenSearch Serverless](https://aws.amazon.com/opensearch-service/features/serverless/): Bedrock Knowledge Bases의 백엔드 데이터베이스 역할을 하며, 전문 검색과 벡터 검색 기능을 제공하여 관련 정보를 정확하게 검색 가능
...
Super-easy Deployment는 내부적으로 CDK를 통한 배포를 수행하기 위해 [AWS CodeBuild](https://aws.amazon.com/codebuild/)를 사용합니다. 이 섹션에서는 CDK를 직접 사용하여 배포하는 절차를 설명합니다.
...
- 필요한 경우 [cdk.json](./cdk/cdk.json)에서 다음 항목을 편집합니다.
  ...
  - `enableLambdaSnapStart`: 기본값은 true입니다. [Python 함수에 대한 Lambda SnapStart를 지원하지 않는 리전](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html#snapstart-supported-regions)에 배포하는 경우 false로 설정하십시오.
  - `globalAvailableModels`: 기본값은 전체입니다. 설정된 경우(모델 ID 목록), Bedrock Chat 애플리케이션에서 모든 사용자의 채팅과 봇 생성 시 드롭다운 메뉴에 나타나는 모델을 전역적으로 제어할 수 있습니다.
  - `logoPath`: 애플리케이션 드로어 상단에 표시되는 이미지를 가리키는 `frontend/public` 아래의 상대 경로.
다음 모델 ID가 지원됩니다(배포 리전의 Bedrock 콘솔에서 Model access에서도 활성화되어 있는지 확인하세요):
...
배포를 위한 파라미터는 `cdk.json`을 사용하거나 타입-세이프한 `parameter.ts` 파일을 사용하는 두 가지 방법으로 정의할 수 있습니다.

#### cdk.json 사용하기 (전통적인 방법)

파라미터를 구성하는 전통적인 방법은 `cdk.json` 파일을 편집하는 것입니다. 이 방법은 간단하지만 타입 체크가 없습니다:
...
#### parameter.ts 사용하기 (권장되는 타입-세이프 방법)
...
1. **스택 이름 지정**:
   ...
   - 하지만 커스텀 봇 스택(`BrChatKbStack*`)과 API 퍼블리싱 스택(`ApiPublishmentStack*`)은 런타임에 동적으로 생성되므로 환경 접두사를 받지 않습니다
...
4. **기본 환경 재정의**: `parameter.ts`에서 "default" 환경을 정의하면 `cdk.json`의 설정을 재정의합니다. `cdk.json`을 계속 사용하려면 `parameter.ts`에서 "default" 환경을 정의하지 마십시오.

5. **환경 요구사항**: "default" 이외의 환경을 생성하려면 `parameter.ts`를 사용해야 합니다. 해당 환경 정의 없이 `-c envName` 옵션만으로는 충분하지 않습니다.

6. **리소스 격리**: 각 환경은 자체 리소스 세트를 생성하므로 동일한 AWS 계정에서 충돌 없이 개발, 테스트 및 프로덕션 환경을 가질 수 있습니다.

## 기타

배포를 위한 매개변수를 정의하는 방법에는 두 가지가 있습니다: `cdk.json`을 사용하는 방법과 타입 안전한 `parameter.ts` 파일을 사용하는 방법입니다.
...
bedrockChatParams.set("prod", {
  ...
  allowedIpV4AddressRanges: ["172.16.0.0/12"],
  enableLambdaSnapStart: true,
  enableRagReplicas: true, // 프로덕션 환경을 위한 향상된 가용성
});
```
...
5. **환경 요구사항**: "default" 이외의 환경을 만들려면 반드시 `parameter.ts`를 사용해야 합니다. 해당하는 환경 정의 없이 `-c envName` 옵션만으로는 충분하지 않습니다.

6. **리소스 격리**: 각 환경은 자체 리소스 세트를 생성하여 동일한 AWS 계정에서 충돌 없이 개발, 테스트, 프로덕션 환경을 가질 수 있습니다.

## 기타

### 리소스 제거

cli와 CDK를 사용하는 경우 `npx cdk destroy`를 실행하세요. 그렇지 않은 경우 [CloudFormation](https://console.aws.amazon.com/cloudformation/home)에 접속하여 `BedrockChatStack`과 `FrontendWafStack`을 수동으로 삭제하세요. `FrontendWafStack`은 `us-east-1` 리전에 있다는 점을 유의하세요.
...
이 샘플은 기본적으로 자체 회원가입이 활성화되어 있습니다. 자체 회원가입을 비활성화하려면 [cdk.json](./cdk/cdk.json)을 열고 `selfSignUpEnabled`를 `false`로 변경하세요. [외부 ID 공급자](#external-identity-provider)를 구성하면 이 값은 무시되고 자동으로 비활성화됩니다.
...
CloudFront 배포의 경우 AWS WAF WebACL은 us-east-1 리전에 생성되어야 합니다. 일부 조직에서는 정책에 의해 기본 리전 외부에 리소스를 생성하는 것이 제한됩니다. 이러한 환경에서는 us-east-1에서 프론트엔드 WAF를 프로비저닝하려고 할 때 CDK 배포가 실패할 수 있습니다.

이러한 제한을 수용하기 위해 프론트엔드 WAF 스택은 선택사항입니다. 비활성화하면 CloudFront 배포는 WebACL 없이 배포됩니다. 이는 프론트엔드 엣지에서 IP 허용/거부 제어가 없다는 것을 의미합니다. 인증 및 기타 모든 애플리케이션 제어는 평소대로 작동합니다. 이 설정은 프론트엔드 WAF(CloudFront 범위)에만 영향을 미치며 게시된 API WAF(지역)는 영향을 받지 않습니다.
...
[cdk.json](./cdk/cdk.json)의 `enableRagReplicas`는 Amazon OpenSearch Serverless를 사용하는 Knowledge Bases의 RAG 데이터베이스에 대한 복제본 설정을 제어하는 옵션입니다.
...
> [!Note]
> 2024년 6월부터 Amazon OpenSearch Serverless는 0.5 OCU를 지원하여 소규모 워크로드의 초기 비용을 낮췄습니다. 프로덕션 배포는 2 OCU로 시작할 수 있으며, 개발/테스트 워크로드는 1 OCU를 사용할 수 있습니다. OpenSearch Serverless는 워크로드 요구사항에 따라 자동으로 확장됩니다. 자세한 내용은 [공지사항](https://aws.amazon.com/jp/about-aws/whats-new/2024/06/amazon-opensearch-serverless-entry-cost-half-collection-types/)을 참조하세요.

### 봇 스토어 구성

봇 스토어 기능을 통해 사용자는 사용자 정의 봇을 공유하고 발견할 수 있습니다. [cdk.json](./cdk/cdk.json)에서 다음 설정을 통해 봇 스토어를 구성할 수 있습니다:
...
- **botStoreLanguage**: 봇 검색과 발견을 위한 기본 언어를 설정합니다(기본값: `"en"`). 이는 봇 스토어에서 봇이 인덱싱되고 검색되는 방식에 영향을 미치며, 지정된 언어에 맞게 텍스트 분석을 최적화합니다.
...
[크로스 리전 및 글로벌 추론](https://docs.aws.amazon.com/bedrock/latest/userguide/inference-profiles-support.html)을 통해 Amazon Bedrock은 여러 AWS 리전에 걸쳐 모델 추론 요청을 동적으로 라우팅하여 피크 수요 기간 동안 처리량과 복원력을 향상시킬 수 있습니다. 글로벌 추론은 전 세계 어디서나 지연 시간과 가용성을 기반으로 최적의 리전으로 요청을 라우팅하며, 크로스 리전 추론은 예를 들어 미국 내에서와 같이 동일한 AWS 리전 내에서 요청을 라우팅합니다. 일부 SCP는 하나 또는 다른 하나 또는 둘 다를 제한할 수 있으므로 이들을 독립적으로 구성할 수 있습니다. 기본적으로 둘 다 활성화되어 있습니다.

`cdk.json` 또는 `parameters.ts`에서 다음 설정을 변경하여 구성하세요:
...
- 사용자 정의 도메인을 사용하도록 CloudFront 구성
...
예를 들어 뉴질랜드 기반 기업은 뉴질랜드(NZ)와 호주(AU)의 IP 주소만 포털에 접근할 수 있도록 하고 다른 모든 접근을 거부하도록 결정할 수 있습니다.
...
프론트엔드는 기본적으로 IP와 IPv6 주소를 모두 받습니다. 드문 경우지만
IPv6 지원을 명시적으로 비활성화해야 할 수 있습니다. 이를 위해
[parameter.ts](./cdk/parameter.ts) 또는 유사하게 [cdk.json](./cdk/cdk.json)에서
다음 매개변수를 설정하세요:
...
이 라이브러리는 MIT-0 라이선스로 제공됩니다. [라이선스 파일](./LICENSE)을 참조하세요.