...
[![개요](https://img.youtube.com/vi/PDTGrHlaLCQ/hq1.jpg)](https://www.youtube.com/watch?v=PDTGrHlaLCQ)
...
고유한 지침을 추가하고 URL 또는 파일로 외부 지식을 제공할 수 있습니다 ([RAG](https://aws.amazon.com/what-is/retrieval-augmented-generation/) 참조). 봇은 애플리케이션 사용자 간에 공유할 수 있으며, 사용자 지정된 봇은 독립형 API로 게시할 수도 있습니다 ([자세히 보기](./PUBLISH_API_ko-KR.md)).
...
> [!Important]
> 거버넌스 이유로 허용된 사용자만 사용자 지정 봇을 만들 수 있습니다. 사용자 지정 봇 생성을 허용하려면 사용자가 `CreatingBotAllowed` 그룹의 구성원이어야 합니다. 이는 관리 콘솔 > Amazon Cognito 사용자 풀 또는 AWS CLI를 통해 설정할 수 있습니다. 사용자 풀 ID는 CloudFormation > BedrockChatStack > 출력 > `AuthUserPoolIdxxxx`에서 확인할 수 있습니다.
...
- us-east-1 리전에서 [Bedrock 모델 접근](https://us-east-1.console.aws.amazon.com/bedrock/home?region=us-east-1#/modelaccess)으로 이동 > `모델 접근 관리` > `Anthropic / Claude 3`, `Amazon / Nova`, `Amazon / Titan Text Embeddings V2`, `Cohere / Embed Multilingual`의 모든 항목을 선택한 후 `변경사항 저장`.
...
- 배포하려는 리전에서 [CloudShell](https://console.aws.amazon.com/cloudshell/home)을 열기
...
- **--disable-self-register**: 자가 등록 비활성화 (기본값: 활성화). 이 플래그를 설정하면 Cognito에서 모든 사용자를 직접 생성해야 하며 사용자의 자체 계정 등록이 허용되지 않습니다.
...
> [!중요]
> 선택적 매개변수를 설정하지 않으면 URL을 아는 사람은 누구나 가입할 수 있습니다. 프로덕션 환경에서는 IP 주소 제한을 추가하고 자가 가입을 비활성화하여 보안 위험을 완화하는 것이 강력히 권장됩니다(회사 도메인의 이메일 주소만 가입할 수 있도록 allowed-signup-email-domains를 정의할 수 있습니다). IP 주소 제한을 위해 ipv4-ranges와 ipv6-ranges를 모두 사용하고, ./bin을 실행할 때 disable-self-register를 사용하여 자가 가입을 비활성화하세요.

> [!팁]
> `프론트엔드 URL`이 표시되지 않거나 Bedrock Claude Chat이 제대로 작동하지 않는 경우, 최신 버전에 문제가 있을 수 있습니다. 이 경우 매개변수에 `--version "v1.2.6"`을 추가하고 다시 배포해 보세요.
...
- [Amazon Bedrock 지식 기반](https://aws.amazon.com/bedrock/knowledge-bases/): 검색 증강 생성(RAG)을 위한 관리형 인터페이스를 제공하며, 문서 임베딩 및 파싱 서비스 제공
...
- [Amazon OpenSearch Serverless](https://aws.amazon.com/opensearch-service/features/serverless/): Bedrock 지식 기반의 백엔드 데이터베이스로 전체 텍스트 검색 및 벡터 검색 기능을 제공하여 관련 정보를 정확하게 검색 가능
...
초간단 배포는 내부적으로 [AWS CodeBuild](https://aws.amazon.com/codebuild/)를 사용하여 CDK로 배포를 수행합니다. 이 섹션에서는 직접 CDK로 배포하는 절차를 설명합니다.
...
- CDK를 배포하기 전에 배포할 리전에 대해 한 번 부트스트래핑해야 합니다.
...
> [!중요]
> 이 프로젝트는 Anthropic Claude 모델에 중점을 두고 있으며, Mistral 모델은 제한적으로 지원됩니다. 예를 들어, 프롬프트 예시는 Claude 모델을 기반으로 합니다. 이는 Mistral 전용 옵션으로, Mistral 모델을 활성화하면 모든 채팅 기능에 대해 Claude와 Mistral 모델을 모두 사용할 수 없고 Mistral 모델만 사용할 수 있습니다.
...
이 샘플은 기본적으로 자체 가입이 활성화되어 있습니다. 자체 가입을 비활성화하려면 [cdk.json](./cdk/cdk.json)을 열고 `selfSignUpEnabled`를 `false`로 전환하세요. [외부 ID 공급자](#external-identity-provider)를 구성하면 이 값은 무시되고 자동으로 비활성화됩니다.
...
이 샘플에는 사용자에게 권한을 부여하는 다음 그룹이 있습니다:
...
[cdk.json](./cdk/cdk.json)의 `enableRagReplicas`는 Amazon OpenSearch Serverless를 사용하는 지식 기반의 RAG 데이터베이스 복제본 설정을 제어하는 옵션입니다.
...
- us-east-1 리전에 DNS 검증을 사용하여 ACM 인증서 생성
