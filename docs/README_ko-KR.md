...
[Amazon Bedrock](https://aws.amazon.com/bedrock/)를 기반으로 하는 다국어 생성형 AI 플랫폼입니다.
...
> [!Warning]
>
> **V3가 출시되었습니다. 업데이트를 위해 [마이그레이션 가이드](./migration/V2_TO_V3_ko-KR.md)를 주의 깊게 검토해 주세요.** 주의하지 않으면 **V2의 봇은 사용할 수 없게 됩니다.**

### 봇 개인화 / 봇 스토어

고유한 지침과 지식(이른바 [RAG](https://aws.amazon.com/what-is/retrieval-augmented-generation/))을 추가할 수 있습니다. 봇은 봇 스토어 마켓플레이스를 통해 애플리케이션 사용자 간에 공유할 수 있습니다. 맞춤형 봇은 독립형 API로도 게시할 수 있습니다 ([자세히 보기](./PUBLISH_API_ko-KR.md)).
...
API 관리, 필수 봇으로 표시, 봇 사용 분석 [자세히 보기](./ADMINISTRATOR_ko-KR.md)
...
- 배포하고자 하는 리전에서 [CloudShell](https://console.aws.amazon.com/cloudshell/home)을 열기
...
- 새 사용자인지 v3를 사용하는지 묻는 메시지가 표시됩니다. v0부터 계속 사용하는 사용자가 아니라면 `y`를 입력하세요.

### 선택적 매개변수

배포 시 보안과 사용자 정의를 향상시키기 위해 다음 매개변수를 지정할 수 있습니다:
...
- **--repo-url**: 포크되거나 사용자 정의된 소스 제어의 Bedrock Chat 사용자 정의 저장소. (기본값: https://github.com/aws-samples/bedrock-chat.git)
...
> [!참고]
> 재정의 값은 AWS 코드 빌드의 배포 시간 동안 기존 cdk.json 구성과 병합됩니다. 재정의에 지정된 값은 cdk.json의 값보다 우선합니다.
...
> [!중요]
> 선택적 매개변수를 설정하지 않으면 URL을 아는 누구나 가입할 수 있습니다. 프로덕션 환경에서는 보안 위험을 완화하기 위해 IP 주소 제한을 추가하고 자체 가입을 비활성화하는 것이 강력히 권장됩니다(회사 도메인의 이메일 주소만 가입할 수 있도록 allowed-signup-email-domains를 정의할 수 있습니다). ./bin을 실행할 때 ipv4-ranges와 ipv6-ranges를 사용하여 IP 주소 제한을 설정하고, disable-self-register를 사용하여 자체 가입을 비활성화하세요.
...
AWS 관리형 서비스를 기반으로 구축된 아키텍처로, 인프라 관리의 필요성을 제거합니다. Amazon Bedrock을 활용함으로써 AWS 외부 API와 통신할 필요가 없습니다. 이를 통해 확장 가능하고, 안정적이며, 보안성 있는 애플리케이션을 배포할 수 있습니다.
...
- [Amazon OpenSearch Serverless](https://aws.amazon.com/opensearch-service/features/serverless/): Bedrock 지식 베이스의 백엔드 데이터베이스로 전체 텍스트 검색 및 벡터 검색 기능 제공, 관련 정보의 정확한 검색 가능
...
Super-easy Deployment는 [AWS CodeBuild](https://aws.amazon.com/codebuild/)를 사용하여 내부적으로 CDK로 배포를 수행합니다. 이 섹션에서는 CDK를 직접 사용하여 배포하는 절차를 설명합니다.

- UNIX, Docker, Node.js 런타임 환경이 필요합니다. 없다면 [Cloud9](https://github.com/aws-samples/cloud9-setup-for-prototyping)을 사용할 수 있습니다.

> [!Important]
> 배포 중 로컬 환경의 저장 공간이 부족하면 CDK 부트스트래핑 과정에서 오류가 발생할 수 있습니다. Cloud9 등에서 실행 중인 경우, 배포 전에 인스턴스의 볼륨 크기를 확장하는 것이 좋습니다.
...
파라미터를 구성하는 전통적인 방법은 `cdk.json` 파일을 편집하는 것입니다. 이 방법은 간단하지만 타입 검사가 없습니다:
...
6. **리소스 격리**: 각 환경은 고유한 리소스 세트를 생성하여 동일한 AWS 계정에서 개발, 테스트, 프로덕션 환경을 충돌 없이 운영할 수 있습니다.

## 기타

배포 매개변수를 정의하는 방법은 두 가지입니다: `cdk.json`을 사용하거나 타입 안전한 `parameter.ts` 파일을 사용하는 것입니다.

#### cdk.json 사용 (기존 방식)

매개변수를 구성하는 기존 방식은 `cdk.json` 파일을 편집하는 것입니다. 이 방식은 간단하지만 타입 검사가 없습니다:
...
이 샘플은 기본적으로 셀프 회원가입이 활성화되어 있습니다. 셀프 회원가입을 비활성화하려면 [cdk.json](./cdk/cdk.json)을 열고 `selfSignUpEnabled`를 `false`로 변경하세요. [외부 ID 공급자](#외부-id-공급자)를 구성하면 이 값은 무시되고 자동으로 비활성화됩니다.
...
[cdk.json](./cdk/cdk.json)의 `enableRagReplicas`는 Amazon OpenSearch Serverless를 사용하는 지식 기반의 복제본 설정을 제어하는 옵션입니다.
...
> [!참고]
> 2024년 6월 현재, Amazon OpenSearch Serverless는 0.5 OCU를 지원하여 소규모 워크로드의 진입 비용을 낮추었습니다. 프로덕션 배포는 2 OCU로 시작할 수 있고, 개발/테스트 워크로드는 1 OCU를 사용할 수 있습니다. OpenSearch Serverless는 워크로드 요구에 따라 자동으로 확장됩니다. 자세한 내용은 [공지사항](https://aws.amazon.com/jp/about-aws/whats-new/2024/06/amazon-opensearch-serverless-entry-cost-half-collection-types/)을 참조하세요.

### 봇 스토어 구성

봇 스토어 기능을 통해 사용자는 맞춤 봇을 공유하고 발견할 수 있습니다. [cdk.json](./cdk/cdk.json)의 다음 설정을 통해 봇 스토어를 구성할 수 있습니다:
...
- **enableBotStore**: 봇 스토어 기능 활성화 여부를 제어합니다(기본값: `true`)
- **botStoreLanguage**: 봇 검색 및 발견을 위한 기본 언어를 설정합니다(기본값: `"en"`). 이는 봇 스토어에서 봇이 색인되고 검색되는 방식에 영향을 미치며, 지정된 언어에 대한 텍스트 분석을 최적화합니다.
- **enableBotStoreReplicas**: 봇 스토어에서 사용되는 OpenSearch Serverless 컬렉션의 대기 복제본 활성화 여부를 제어합니다(기본값: `false`). `true`로 설정하면 가용성이 향상되지만 비용이 증가하고, `false`로 설정하면 비용이 절감되지만 가용성에 영향을 줄 수 있습니다.
  > **중요**: 컬렉션이 이미 생성된 후에는 이 속성을 업데이트할 수 없습니다. 이 속성을 수정하려고 시도하면 컬렉션은 원래 값을 계속 사용합니다.

### 크로스 리전 추론

[크로스 리전 추론](https://docs.aws.amazon.com/bedrock/latest/userguide/inference-profiles-support.html)을 통해 Amazon Bedrock은 여러 AWS 리전에서 모델 추론 요청을 동적으로 라우팅하여 피크 수요 기간 동안 처리량과 복원력을 향상시킬 수 있습니다. 구성하려면 `cdk.json`을 편집하세요.
...
[cdk.json](./cdk/cdk.json)에서 다음 매개변수를 설정하여 CloudFront 배포의 맞춤 도메인을 구성할 수 있습니다:
...
이 라이브러리는 MIT-0 라이선스에 따라 라이선스가 부여됩니다. [라이선스 파일](./LICENSE)을 참조하세요.