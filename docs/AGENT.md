...
This tool depends [DuckDuckGo](https://duckduckgo.com/) which has rate limit. It's suitable for PoC or demo purpose. Bing Search is provided as an alternative when DuckDuckGo API fails. For production use modify this tool as needed to match your needs. To utilize Bing Search you need to create and setup BING Search API Key in AWS Secrets. AWS Command line equivalent for this:  
```
read -s BING_API_KEY && aws secretsmanager create-secret \
  --name "bing-api-key" \
  --secret-string "{\"search_api_key\":\"$BING_API_KEY\"}"
```
