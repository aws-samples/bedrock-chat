...
Un utilisateur demande "le total des ventes pour le dernier trimestre". L'Agent interprète cette demande, la convertit en requête SQL, l'exécute dans la base de données et présente les résultats.

### Prévisions financières

Un analyste financier doit prévoir les revenus du prochain trimestre. L'Agent rassemble les données pertinentes, effectue les calculs nécessaires en utilisant des modèles financiers et génère un rapport de prévision détaillé, en garantissant la précision des projections.
...
def calculator(expression: str) -> dict:
    """
    Effectue des calculs mathématiques en toute sécurité.
...
def create_calculator_tool(bot: BotModel | None = None):
    """Crée un outil calculatrice avec une closure de contexte bot."""
...
- Utilisez `{"json": data}` pour les données complexes qui doivent être préservées comme information structurée
- Définissez toujours `status` soit à `"success"` soit à `"error"`

### Directives d'implémentation

- Le nom de la fonction et la docstring sont utilisés lorsque le LLM décide quel outil utiliser. La docstring est intégrée dans le prompt, donc décrivez précisément l'objectif et les paramètres de l'outil.
...
- [Optionnel] Ajoutez des noms et descriptions clairs pour le frontend. Cette étape est optionnelle, mais si vous ne la faites pas, le nom et la description de votre fonction seront utilisés. Comme ceux-ci sont destinés à la consommation LLM, il est recommandé d'ajouter des explications conviviales pour une meilleure expérience utilisateur.

  - Modifiez les fichiers i18n. Ouvrez [en/index.ts](../frontend/src/i18n/en/index.ts) et ajoutez vos propres `name` et `description` dans `agent.tools`.
