...
Jeśli już korzystasz z Bedrock Chat w poprzedniej wersji (~`0.4.x`), musisz postępować zgodnie z poniższymi krokami, aby przeprowadzić migrację.
...
- Pobierz branch, który chcesz wdrożyć. Przełącz się na pożądany branch (w tym przypadku `v1`) i pobierz najnowsze zmiany:
...
- Jeśli chcesz przywrócić elementy za pomocą DMS, KONIECZNIE wyłącz rotację hasła i zanotuj hasło dostępu do bazy danych. Przy użyciu skryptu migracyjnego ([migrate_v0_v1.py](./migrate_v0_v1.py)) nie musisz notować hasła.
...
- Jeśli chcesz, aby konkretni użytkownicy mogli tworzyć botów, dodaj tych użytkowników do grupy `CreatingBotAllowed` za pomocą konsoli zarządzania lub interfejsu wiersza poleceń.
