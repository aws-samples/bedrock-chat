...
Jeśli już używasz Bedrock Claude Chat w poprzedniej wersji (~`0.4.x`), musisz postępować zgodnie z poniższymi krokami, aby przeprowadzić migrację.
...
- Pobierz branch, który chcesz wdrożyć. Przełącz się na wybrany branch (w tym przypadku `v1`) i pobierz najnowsze zmiany:
...
- Jeśli chcesz przywrócić elementy za pomocą DMS, PAMIĘTAJ o wyłączeniu rotacji hasła i zanotuj hasło dostępu do bazy danych. Jeśli przywracasz za pomocą skryptu migracji ([migrate.py](./migrate.py)), nie musisz zapamiętywać hasła.
