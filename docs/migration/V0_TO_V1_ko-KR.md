...
- 봇 생성 가능한 사용자를 제한하기 위해 `CreatingBotAllowed` Cognito 사용자 그룹을 도입했습니다. 현재 기존 사용자는 이 그룹에 속해 있지 않으므로, 봇 생성 권한을 부여하려면 수동으로 권한을 첨부해야 합니다. 참조: [봇 개인화](../../README.md#bot-personalization)
...
- 배포하려는 브랜치를 풀합니다. 원하는 브랜치(이 경우 `v1`)로 전환하고 최신 변경 사항을 풀합니다:
...
- DMS로 항목을 복원하려면 비밀번호 회전을 비활성화하고 데이터베이스에 액세스할 비밀번호를 메모해야 합니다. 마이그레이션 스크립트([migrate.py](./migrate.py))로 복원하는 경우 비밀번호를 메모할 필요가 없습니다.
...
- 특정 사용자가 봇을 생성할 수 있도록 하려면 관리 콘솔 또는 CLI를 사용하여 해당 사용자를 `CreatingBotAllowed` 그룹에 추가합니다.
