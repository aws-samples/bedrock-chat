...
本指南概述了在更新 Bedrock Chat（包含 Aurora 叢集替換）時遷移資料的步驟。以下程序確保順利過渡，同時將停機時間和資料損失降至最低。

## 概述

遷移過程包括掃描所有機器人並為每個機器人啟動嵌入式 ECS 任務。這種方法需要重新計算嵌入向量，這可能會耗費較長時間，並因為執行 ECS 任務和使用 Bedrock Cohere 而產生額外費用。如果您想避免這些成本和時間消耗，請參考本指南後面提供的[替代遷移方案](#alternative-migration-options)。

## 遷移步驟

- 使用 Aurora 替換後執行 [npx cdk deploy](../README.md#deploy-using-cdk)，打開 [migrate_v0_v1.py](./migrate_v0_v1.py) 腳本並更新以下變數為適當的值。這些值可以在 `CloudFormation` > `BedrockChatStack` > `Outputs` 標籤頁中找到。

```py
# 在 AWS Management Console 中打開 CloudFormation 堆疊並從 Outputs 標籤頁複製值。
...
- 執行 `migrate_v0_v1.py` 腳本以開始遷移程序。此腳本將掃描所有機器人、啟動嵌入式 ECS 任務，並在新的 Aurora 叢集中建立資料。請注意：
  - 此腳本需要 `boto3`。
  - 環境需要具有存取 dynamodb 資料表和調用 ECS 任務的 IAM 權限。
...
注意：截至 2024 年 5 月 29 日，DMS 並不原生支援 pgvector 擴充功能。不過，你可以探索以下方案來解決這個限制：
