...
- 向量資料庫（即 Aurora PostgreSQL 上的 pgvector）儲存現已加密，部署時將觸發替換。這意味著現有的向量項目將被刪除。
- 我們引入了 `CreatingBotAllowed` Cognito 使用者群組，以限制可以建立機器人的使用者。現有的使用者預設不在此群組中，因此如果您希望他們具有建立機器人的權限，需要手動附加權限。請參閱：[Bot 個人化](../../README.md#bot-personalization)
...
- 打開終端機並導航到專案目錄
- 拉取您想要部署的分支。切換到所需分支（在此情況下為 `v1`）並拉取最新的變更：
