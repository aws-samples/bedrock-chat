...
- V3 introduit un contrôle des permissions détaillé et une fonctionnalité Bot Store, nécessitant des modifications du schéma DynamoDB
...
- **IMPORTANT : Toutes les APIs publiées doivent être supprimées pendant le processus de migration.**
...
1. **Contrôle précis des permissions** : Contrôlez l'accès à vos bots avec des permissions basées sur les groupes d'utilisateurs
2. **Bot Store** : Partagez et découvrez des bots via une place de marché centralisée
3. **Fonctionnalités administratives** : Gérez les API, marquez les bots comme essentiels et analysez l'utilisation des bots
...
> [!IMPORTANT]
> Avant de procéder à la migration : **Le processus de migration ne peut pas garantir un succès à 100% pour tous les bots**, en particulier ceux créés avec des versions plus anciennes ou avec des configurations personnalisées. Veuillez documenter vos configurations de bot importantes (instructions, sources de connaissances, paramètres) avant de commencer le processus de migration au cas où vous devriez les recréer manuellement.

## Processus de Migration

### Avis Important Concernant la Visibilité des Bots dans V3

Dans V3, **tous les bots v2 avec le partage public activé seront recherchables dans le Bot Store.** Si vous avez des bots contenant des informations sensibles que vous ne souhaitez pas rendre découvrables, envisagez de les rendre privés avant de migrer vers V3.
...
### Étape 6 : Supprimer toutes les APIs publiées

> [!IMPORTANT]
> Avant de déployer V3, vous devez supprimer toutes les APIs publiées pour éviter les conflits de valeurs de sortie Cloudformation pendant le processus de mise à niveau.
...
3. Examinez la liste de toutes les APIs publiées
4. Supprimez chaque API publiée en cliquant sur le bouton de suppression à côté

Vous pouvez trouver plus d'informations sur la publication et la gestion des APIs dans la documentation [PUBLISH_API.md](../PUBLISH_API_fr-FR.md), [ADMINISTRATOR.md](../ADMINISTRATOR_fr-FR.md) respectivement.
...
> [!IMPORTANT]
> Une fois que vous déployez V3, l'application sera inaccessible à tous les utilisateurs jusqu'à ce que le processus de migration soit terminé. Le nouveau schéma est incompatible avec l'ancien format de données, donc les utilisateurs ne pourront pas accéder à leurs bots ou conversations jusqu'à ce que vous complétiez le script de migration dans les prochaines étapes.
...
> [!Important]
> Assurez-vous de sauvegarder ces noms de tables V3 avec votre nom de table V2 précédemment enregistré, car vous aurez besoin de tous pour le script de migration.
...
> [!NOTE]
> La version requise de Python a été changée à 3.13.0 ou ultérieure (Peut être modifiée dans le développement futur. Voir pyproject.toml). Si vous avez venv installé avec une version Python différente, vous devrez le supprimer une fois.
...
Le script de migration générera un fichier rapport dans votre répertoire courant avec les détails du processus de migration. Vérifiez ce fichier pour vous assurer que toutes vos données ont été migrées correctement.
...
3. **Surveiller la migration** : Vérifiez les fichiers rapport générés pour vous assurer que toutes les données sont migrées correctement, en particulier pour les grands ensembles de données.
...
> [!IMPORTANT]
> Ne supprimez la table V2 qu'après avoir vérifié minutieusement que toutes vos données importantes ont été migrées avec succès vers V3. Nous recommandons de conserver la sauvegarde créée à l'Étape 2 pendant au moins quelques semaines après la migration, même si vous supprimez la table originale.
...
R : L'autorisation est vérifiée au moment du chat, donc vous perdrez l'accès immédiatement.

**Q : Que se passe-t-il si un utilisateur est supprimé (par exemple, un employé qui part) ?**
R : Ses données peuvent être complètement supprimées en effaçant tous les éléments de DynamoDB ayant son ID utilisateur comme clé de partition (PK).
...
R : Pas dans la version initiale V3. Cependant, les éléments peuvent toujours être supprimés en interrogeant avec l'ID utilisateur si nécessaire.

**Q : Y aura-t-il une fonctionnalité d'étiquetage des bots pour une meilleure expérience de recherche ?**
