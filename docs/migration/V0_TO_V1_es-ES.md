...
Si ya está utilizando Bedrock Chat con una versión anterior (~`0.4.x`), necesita seguir los siguientes pasos para migrar.
...
- Extrae la rama que deseas implementar. A continuación, cambia a la rama deseada (en este caso, `v1`) y extrae los últimos cambios:
...
- Si deseas restaurar elementos con DMS, NO OLVIDES deshabilitar la rotación de contraseñas y anotar la contraseña para acceder a la base de datos. Si se restaura con el script de migración([migrate_v0_v1.py](./migrate_v0_v1.py)), no necesitas anotar la contraseña.
...
- Si deseas que usuarios específicos puedan crear bots, agrega esos usuarios al grupo `CreatingBotAllowed` utilizando la consola de administración o la CLI.
