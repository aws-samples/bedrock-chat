...
Hướng dẫn này mô tả các bước để di chuyển dữ liệu khi thực hiện cập nhật Bedrock Claude Chat, bao gồm việc thay thế cụm Aurora. Quy trình sau sẽ đảm bảo quá trình chuyển đổi diễn ra suôn sẻ, giảm thiểu thời gian ngừng hoạt động và mất mát dữ liệu.
...
- Chạy tập lệnh `migrate.py` để bắt đầu quá trình di chuyển. Tập lệnh này sẽ quét tất cả các bot, khởi chạy các tác vụ nhúng ECS và tạo dữ liệu vào cụm Aurora mới. Lưu ý rằng:
  ...
  - Môi trường cần có quyền IAM để truy cập bảng dynamodb và khởi chạy các tác vụ ECS.
