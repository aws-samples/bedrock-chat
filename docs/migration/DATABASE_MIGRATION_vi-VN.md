...
- Sau khi [npx cdk deploy](../README.md#deploy-using-cdk) với việc thay thế Aurora, hãy mở tập lệnh [migrate.py](./migrate.py) và cập nhật các biến sau với các giá trị thích hợp. Các giá trị có thể được tham chiếu từ tab `CloudFormation` > `BedrockChatStack` > `Outputs`.
...
- Chạy tập lệnh `migrate.py` để bắt đầu quá trình di chuyển. Tập lệnh này sẽ quét tất cả các bot, khởi chạy các tác vụ ECS nhúng và tạo dữ liệu vào cụm Aurora mới. Lưu ý rằng:
  - Tập lệnh yêu cầu `boto3`.
  - Môi trường cần các quyền IAM để truy cập bảng dynamodb và khởi chạy các tác vụ ECS.
...
Trước tiên, lưu ý mật khẩu để truy cập cụm Aurora hiện tại. Sau đó chạy `npx cdk deploy`, điều này sẽ kích hoạt việc thay thế cụm. Tiếp theo, tạo một cơ sở dữ liệu tạm thời bằng cách khôi phục từ ảnh chụp của cơ sở dữ liệu ban đầu.
...
Sử dụng [di chuyển đồng nhất DMS](https://docs.aws.amazon.com/dms/latest/userguide/dm-migrating-data.html), tận dụng sao chép logic gốc. Trong trường hợp này, cả cơ sở dữ liệu nguồn và đích đều phải là PostgreSQL. DMS có thể tận dụng sao chép logic gốc cho mục đích này.
