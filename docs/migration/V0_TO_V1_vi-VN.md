...
- Kho lưu trữ cơ sở dữ liệu vector (tức là pgvector trên Aurora PostgreSQL) hiện đã được mã hóa, điều này sẽ kích hoạt việc thay thế khi triển khai. Điều này có nghĩa các mục vector hiện có sẽ bị xóa.
- Chúng tôi đã giới thiệu nhóm người dùng Cognito `CreatingBotAllowed` để hạn chế người dùng có thể tạo bot. Người dùng hiện có không thuộc nhóm này, vì vậy bạn cần gắn quyền một cách thủ công nếu muốn họ có khả năng tạo bot. Xem: [Bot Personalization](../../README.md#bot-personalization)
