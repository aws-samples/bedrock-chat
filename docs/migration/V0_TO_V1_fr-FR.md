...
- Le stockage de la base de données vectorielle (c'est-à-dire pgvector sur Aurora PostgreSQL) est maintenant chiffré, ce qui déclenche un remplacement lors du déploiement. Cela signifie que les éléments vectoriels existants seront supprimés.
- Nous avons introduit le groupe d'utilisateurs Cognito `CreatingBotAllowed` pour limiter les utilisateurs pouvant créer des bots. Les utilisateurs existants ne font pas partie de ce groupe, vous devrez donc attacher manuellement l'autorisation si vous souhaitez qu'ils aient des capacités de création de bot. Voir : [Personnalisation du Bot](../../README.md#bot-personalization)
...
- Tirez la branche que vous souhaitez déployer. Basculez vers la branche désirée (dans ce cas, `v1`) et récupérez les dernières modifications :
...
- Après le déploiement, tous les utilisateurs ne pourront plus créer de nouveaux bots.
