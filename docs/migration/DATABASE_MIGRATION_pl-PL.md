...
Proces migracji polega na przeskanowaniu wszystkich botów i uruchomieniu zadań ECS z osadzaniem dla każdego z nich. Takie podejście wymaga przeliczenia osadzeń, co może być czasochłonne i wiązać się z dodatkowymi kosztami ze względu na wykonanie zadań ECS oraz opłaty za korzystanie z usługi Bedrock Cohere. Jeśli wolisz uniknąć tych kosztów i wymagań czasowych, zapoznaj się z [alternatywnymi opcjami migracji](#alternative-migration-options), które zostały przedstawione w dalszej części tego przewodnika.

## Kroki migracji

- Po [npx cdk deploy](../README.md#deploy-using-cdk) z wymianą Aurora, otwórz skrypt [migrate.py](./migrate.py) i zaktualizuj następujące zmienne odpowiednimi wartościami. Wartości można znaleźć w zakładce `CloudFormation` > `BedrockChatStack` > `Outputs`.
