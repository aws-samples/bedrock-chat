...
如果您已经使用之前版本的 Bedrock Claude Chat（~`0.4.x`），则需要按照以下步骤进行迁移。

## 为什么需要这样做？

这次重大更新包括重要的安全更新。

- 向量数据库（即 Aurora PostgreSQL 上的 pgvector）存储现已加密，部署时将触发替换。这意味着现有的向量项将被删除。
...
- 打开终端并导航到项目目录
- 拉取您想要部署的分支。切换到所需分支（在本例中是 `v1`）并拉取最新的更改：
