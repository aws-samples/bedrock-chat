# Database-migreringsguide

> [!Warning]
> Denne guiden er for v0 til v1.

Denne guiden beskriver trinnene for å migrere data når man utfører en oppdatering av Bedrock Chat som inneholder en Aurora-klusterutskiftning. Følgende prosedyre sikrer en smidig overgang samtidig som nedetid og datatap minimeres.

## Oversikt

Migrasjonsprosessen innebærer skanning av alle boter og oppstart av innbyggings-ECS-oppgaver for hver av dem. Denne tilnærmingen krever ny beregning av innbygginger, noe som kan være tidkrevende og medføre ekstra kostnader på grunn av ECS-oppgavekjøring og Bedrock Cohere-bruksgebyrer. Hvis du foretrekker å unngå disse kostnadene og tidsbruken, vennligst se [alternative migrasjonsalternativer](#alternative-migration-options) som er beskrevet senere i denne veiledningen.
...
CONTAINER_NAME = "Container"  # Trenger ikke endres
...
- Kjør `migrate_v0_v1.py` skriptet for å starte migreringsprosessen. Dette skriptet vil skanne alle boter, starte embedding ECS-oppgaver, og opprette dataene i den nye Aurora-clusteren. Merk at:
  ...
  - Miljøet krever IAM-tillatelser for å få tilgang til dynamodb-tabellen og for å kjøre ECS-oppgaver.
...
### Gjenoppretting fra sikkerhetskopi og DMS-migrasjon

Først, noter passordet for å få tilgang til nåværende Aurora-cluster. Kjør deretter `npx cdk deploy`, som utløser utskifting av clusteret. Etter det, opprett en midlertidig database ved å gjenopprette fra en sikkerhetskopi av den originale databasen.
