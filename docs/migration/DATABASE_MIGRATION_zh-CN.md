...
本指南概述了在更新包含 Aurora 集群替换的 Bedrock Chat 时进行数据迁移的步骤。以下步骤确保在最小化停机时间和数据丢失的情况下顺利过渡。
...
- 在使用 Aurora 替换执行 [npx cdk deploy](../README.md#deploy-using-cdk) 后，打开 [migrate_v0_v1.py](./migrate_v0_v1.py) 脚本，并使用适当的值更新以下变量。这些值可以在 `CloudFormation` > `BedrockChatStack` > `Outputs` 选项卡中查找。
...
- 运行 `migrate_v0_v1.py` 脚本以启动迁移过程。此脚本将扫描所有机器人，启动嵌入式 ECS 任务，并将数据创建到新的 Aurora 集群。请注意：
  - 该脚本需要 `boto3`。
