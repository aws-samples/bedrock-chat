...
- Introduzimos o grupo de usuários do Cognito `CreatingBotAllowed` para limitar os usuários que podem criar bots. Os usuários existentes não estão neste grupo, então você precisa atribuir a permissão manualmente se quiser que eles tenham capacidades de criação de bots. Veja: [Personalização de Bot](../../README.md#bot-personalization)
...
- Faça pull da branch que deseja implantar. A seguir, vá para a branch desejada (neste caso, `v1`) e faça pull das últimas alterações:
