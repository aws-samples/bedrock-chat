...
Trên màn hình tiếp theo, chúng ta có thể cấu hình một số tham số liên quan đến giới hạn tốc độ. Để biết chi tiết, vui lòng xem thêm: [Giới hạn tốc độ yêu cầu API để cải thiện thông lượng](https://docs.aws.amazon.com/apigateway/latest/developerguide/api-gateway-request-throttling.html).
...
Khi người dùng nhấp vào nút xuất bản bot, [AWS CodeBuild](https://aws.amazon.com/codebuild/) sẽ khởi chạy nhiệm vụ triển khai CDK để cung cấp ngăn xếp API (Xem thêm: [Định nghĩa CDK](../cdk/lib/api-publishment-stack.ts)) bao gồm API Gateway, Lambda và SQS. SQS được sử dụng để tách rời yêu cầu người dùng và hoạt động LLM vì việc tạo đầu ra có thể vượt quá 30 giây, là giới hạn của ngưỡng API Gateway. Để tìm nạp đầu ra, cần truy cập API một cách không đồng bộ. Để biết thêm chi tiết, xem [Thông số kỹ thuật API](#api-specification).
