...
管理员用户必须是名为 `Admin` 的组的成员，可以通过管理控制台 > Amazon Cognito 用户池或 AWS CLI 设置。请注意，可以通过访问 CloudFormation > BedrockChatStack > 输出 > `AuthUserPoolIdxxxx` 来引用用户池 ID。

![](./imgs/group_membership_admin.png)

## 将公共机器人标记为"必不可少"

管理员现在可以将公共机器人标记为"必不可少"。标记为"必不可少"的机器人将在机器人商店的"必不可少"分区中突出显示，使用户可以轻松访问。这使管理员能够置顶他们希望所有用户使用的重要机器人。

### 示例

- HR助理机器人：帮助员工解决人力资源相关的问题和任务。
- IT支持机器人：为内部技术问题和账户管理提供帮助。
- 内部政策指南机器人：解答关于考勤规则、安全政策和其他内部规定的常见问题。
- 新员工入职机器人：指导新员工在第一天完成流程并使用系统。
...
来自大语言模型（LLM）的输出可能并不总是符合用户的期望。有时无法满足用户的需求。为了有效地将大语言模型"集成"到业务运营和日常生活中，实施反馈循环是至关重要的。Bedrock Chat配备了反馈功能，旨在使用户能够分析不满意的原因。基于分析结果，用户可以相应地调整提示词、RAG数据源和参数。
...
目前提供了聊天机器人和用户使用情况的基本概述，主要关注在特定时间段内对每个机器人和用户的数据进行汇总，并按使用费用对结果进行排序。
...
> [!Important]
> 如果您使用多个环境（开发、生产等），Athena 数据库名称将包括环境前缀。不是 `bedrockchatstack_usage_analysis`，数据库名称将是：
>
...
编辑 `bot-id` 和 `datehour`。`bot-id` 可以在机器人管理屏幕上查看，该屏幕可以通过机器人发布 API 访问，显示在左侧边栏上。注意 URL 末尾的部分，如 `https://xxxx.cloudfront.net/admin/bot/<bot-id>`。
