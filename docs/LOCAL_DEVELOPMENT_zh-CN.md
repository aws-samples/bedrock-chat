...
我们引入了用于类型检查和代码检查的 GitHub 工作流。这些工作流在创建 Pull Request 时执行，但等待代码检查完成后再继续并不是一个良好的开发体验。因此，这些代码检查任务应在提交阶段自动执行。我们引入了 [Lefthook](https://github.com/evilmartians/lefthook?tab=readme-ov-file#install) 作为实现这一目标的机制。这不是强制性的，但我们建议采用它以获得高效的开发体验。另外，尽管我们不强制使用 [Prettier](https://prettier.io/) 进行 TypeScript 格式化，但我们希望你在贡献代码时能够采用它，因为这有助于防止代码审查时出现不必要的差异。
