...
Một Tác Nhân sử dụng ReAct có thể được áp dụng trong nhiều tình huống khác nhau, mang lại các giải pháp chính xác và hiệu quả.

### Chuyển Đổi Văn Bản Sang SQL

Người dùng yêu cầu "tổng doanh số bán hàng của quý vừa qua". Tác Nhân hiểu yêu cầu này, chuyển đổi nó thành một truy vấn SQL, thực thi truy vấn trên cơ sở dữ liệu, và trình bày kết quả.
...
Để kích hoạt chức năng Agent cho chatbot được tùy chỉnh của bạn, hãy thực hiện các bước sau:
...
3. Để kích hoạt một công cụ, chỉ cần bật công tắc bên cạnh công cụ mong muốn. Sau khi một công cụ được kích hoạt, Agent sẽ có quyền truy cập và có thể sử dụng nó khi xử lý các truy vấn của người dùng.

![](./imgs/agent_tools.png)

> [!Quan trọng]
> Điều quan trọng cần lưu ý là việc kích hoạt bất kỳ công cụ nào trong phần Agent sẽ tự động coi chức năng "Kiến thức" như một công cụ. Điều này có nghĩa là LLM sẽ tự động xác định xem có nên sử dụng "Kiến thức" để trả lời các truy vấn của người dùng hay không, coi nó như một trong những công cụ có sẵn.
...
- Tham khảo triển khai mẫu của [công cụ tính BMI](../examples/agents/tools/bmi/bmi.py). Ví dụ này cho thấy cách tạo một công cụ tính Chỉ số Khối Cơ Thể (BMI) dựa trên đầu vào của người dùng.
  ...
  - Tên và mô tả được khai báo trên công cụ được sử dụng khi LLM xem xét công cụ nào sẽ được sử dụng để trả lời câu hỏi của người dùng. Nói cách khác, chúng được nhúng vào lời nhắc khi gọi LLM. Do đó, việc mô tả càng chính xác càng tốt.

- [Tùy chọn] Sau khi đã triển khai công cụ tùy chỉnh của mình, bạn nên xác minh chức năng của nó bằng tập lệnh kiểm tra ([ví dụ](../examples/agents/tools/bmi/test_bmi.py)). Tập lệnh này sẽ giúp bạn đảm bảo rằng công cụ của mình hoạt động như mong đợi.

- Sau khi hoàn tất phát triển và kiểm tra công cụ tùy chỉnh, hãy di chuyển tệp triển khai đến thư mục [backend/app/agents/tools/](../backend/app/agents/tools/). Sau đó mở [backend/app/agents/utils.py](../backend/app/agents/utils.py) và chỉnh sửa `get_available_tools` để người dùng có thể chọn công cụ đã phát triển.

- [Tùy chọn] Thêm tên và mô tả rõ ràng cho giao diện người dùng. Bước này là tùy chọn, nhưng nếu bạn không thực hiện bước này, tên và mô tả công cụ được khai báo trong công cụ của bạn sẽ được sử dụng. Chúng dành cho LLM nhưng không phải cho người dùng, vì vậy việc thêm giải thích chuyên dụng được khuyến nghị để cải thiện trải nghiệm người dùng.
...
- Chạy `npx cdk deploy` để triển khai các thay đổi của bạn. Điều này sẽ làm cho công cụ tùy chỉnh của bạn khả dụng trên màn hình bot tùy chỉnh.
