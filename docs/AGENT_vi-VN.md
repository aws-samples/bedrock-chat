...
Một người dùng yêu cầu "tổng doanh số bán hàng của quý trước." Agent diễn giải yêu cầu này, chuyển đổi thành truy vấn SQL, thực thi trên cơ sở dữ liệu và trình bày kết quả.
...
3. Để kích hoạt một công cụ, chỉ cần bật công tắc bên cạnh công cụ mong muốn. Khi công cụ được kích hoạt, Agent sẽ có quyền truy cập và có thể sử dụng nó khi xử lý các truy vấn của người dùng.
...
Đầu tiên, tạo một Agent trong Bedrock (ví dụ: thông qua Management Console). Sau đó, chỉ định ID Agent trong màn hình cài đặt bot tùy chỉnh. Sau khi thiết lập, chatbot của bạn sẽ tận dụng Bedrock Agent để xử lý các truy vấn của người dùng.
...
Strands cung cấp decorator `@tool` đơn giản để chuyển đổi các hàm Python thông thường thành công cụ cho AI agent. Decorator tự động trích xuất thông tin từ docstring và type hints của hàm để tạo ra các đặc tả công cụ mà LLM có thể hiểu và sử dụng. Cách tiếp cận này tận dụng các tính năng gốc của Python để tạo ra trải nghiệm phát triển công cụ sạch sẽ và chức năng.

Để biết thông tin chi tiết về công cụ Strands, xem [Python Tools documentation](https://strandsagents.com/latest/documentation/docs/user-guide/concepts/tools/python-tools/).
...
### Công cụ với Bot Context (Closure Pattern)

Để truy cập thông tin bot (BotModel), sử dụng closure pattern để nắm bắt ngữ cảnh của bot:
...
- Sử dụng `{"json": data}` cho dữ liệu phức tạp cần được giữ nguyên dưới dạng thông tin có cấu trúc
- Luôn đặt `status` là `"success"` hoặc `"error"`

### Hướng dẫn Triển khai

- Tên hàm và docstring được sử dụng khi LLM xem xét công cụ nào để sử dụng. Docstring được nhúng vào prompt, vì vậy hãy mô tả mục đích và tham số của công cụ một cách chính xác.

- Tham khảo mẫu triển khai của [công cụ tính BMI](../examples/agents/tools/bmi/bmi_strands.py). Ví dụ này minh họa cách tạo công cụ tính Chỉ số Khối cơ thể (BMI) bằng decorator `@tool` của Strands và closure pattern.
...
- [Tùy chọn] Thêm tên và mô tả rõ ràng cho frontend. Bước này là tùy chọn, nhưng nếu bạn không thực hiện bước này, tên và mô tả công cụ từ hàm của bạn sẽ được sử dụng. Vì những thông tin này dành cho LLM sử dụng, nên bạn nên thêm giải thích thân thiện với người dùng để có UX tốt hơn.

  - Chỉnh sửa file i18n. Mở [en/index.ts](../frontend/src/i18n/en/index.ts) và thêm `name` và `description` của bạn vào `agent.tools`.
  - Chỉnh sửa `xx/index.ts` tương tự. Trong đó `xx` đại diện cho mã quốc gia bạn muốn.

- Chạy `npx cdk deploy` để triển khai thay đổi của bạn. Điều này sẽ làm cho công cụ tùy chỉnh của bạn có sẵn trong màn hình bot tùy chỉnh.