...
Trong mẫu này, bạn có thể sửa đổi và khởi chạy frontend một cách cục bộ bằng cách sử dụng các tài nguyên AWS (`API Gateway`, `Cognito`, v.v.) đã được triển khai bằng lệnh `npx cdk deploy`.
...
## (Tùy chọn, được khuyến nghị) Thiết lập hook pre-commit

Chúng tôi đã giới thiệu các GitHub workflows để kiểm tra kiểu và kiểm tra mã. Những việc này được thực hiện khi một Pull Request được tạo, nhưng việc chờ đợi kiểm tra mã hoàn tất trước khi tiếp tục không phải là một trải nghiệm phát triển tốt. Do đó, các tác vụ kiểm tra mã này nên được thực hiện tự động tại giai đoạn commit. Chúng tôi đã giới thiệu [Lefthook](https://github.com/evilmartians/lefthook?tab=readme-ov-file#install) như một cơ chế để đạt được điều này. Việc này không bắt buộc, nhưng chúng tôi khuyến nghị áp dụng nó để có trải nghiệm phát triển hiệu quả. Ngoài ra, mặc dù chúng tôi không buộc định dạng TypeScript bằng [Prettier](https://prettier.io/), nhưng chúng tôi rất mong bạn có thể áp dụng nó khi đóng góp, vì nó giúp ngăn các thay đổi không cần thiết trong quá trình đánh giá mã.
